...
export const ListProjectsQueryParams = zod.object({
  ...
  "status": zod.array(zod.enum(['active', 'paused', 'completed', 'archived']).describe('completed 是“标记完成”命令和历史兼容值。服务端完成未完成事项确认后会\n直接把 Project 写为 archived，不长期返回 completed 分组。\n')).optional().describe('不传时默认只返回 active 和 paused。'),
...
export const UpdateProjectBody = zod.object({
  ...
  "description": zod.string().nullish(),
  "status": zod.enum(['active', 'paused', 'completed', 'archived']).optional().describe('completed 是“标记完成”命令和历史兼容值。服务端完成未完成事项确认后会\n直接把 Project 写为 archived，不长期返回 completed 分组。\n'),
  "start_date": zod.string().date().nullish(),
  "target_date": zod.string().date().nullish(),
