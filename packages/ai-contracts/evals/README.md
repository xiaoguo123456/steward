...
尚未因合规门禁开放运行时的敏感能力使用 `runner: "contract"`。这类用例不传入
Assistant 编排栈，而是确定性检查 Prompt 与 JSON Schema 是否仍保留未信任正文、
选择范围、来源字段、诊断禁令和输出上限。它不能替代真实 Provider 质量 Eval；
运行时正式开放时必须新增专用执行器，再把模型输出质量纳入基线。
