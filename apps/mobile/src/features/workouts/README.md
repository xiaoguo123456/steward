...
- 进入户外运动页后采集 GPS；首次获得精度不差于 30 米的有效坐标前不计时、不累计路线，定位成功并完成语音倒数后才开始运动。
- 暂停、打开结束确认或卸载页面时停止采集，恢复后开启新路线段。
- 过滤精度差于 50 米、原地漂移、不可能速度和超过 30 秒的断档点。有效坐标用于绘制路线并计算距离、平均配速或平均速度。
- 开启语音后使用设备 TTS 播报开始、暂停、继续、整公里、目标完成和结束；文案由确定性代码生成，跑步过程中不调用在线模型。
- 用户开启“运动中保持屏幕常亮”后，进行中页面通过 `expo-keep-awake` 持有常亮状态，离开页面时由组件生命周期自动释放。该能力属于原生运行时变更，不能只通过 OTA 交付。
...
总结页只有在正式 Record 保存成功后才清理本次运动的准备、进行中和总结页历史并返回原运动首页；保存失败保留距离、感受和当前页面供用户重试。主动放弃仍直接返回。
