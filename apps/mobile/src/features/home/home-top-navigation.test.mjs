...
test('亲友回到首页分区且正式构建不展示示例人物', async () => {
  ...
  assert.match(relationships, /label="添加亲友"/);
...
test('四个首页分区复用统一紧凑按钮及全局操作色', async () => {
  const memories = await readFile(new URL('../memories/memories-home.tsx', import.meta.url), 'utf8');
  ...
  for (const source of [memories, relationships, inspiration, mood]) {
  ...
  assert.match(relationships, /label="添加亲友"[\s\S]*variant="secondary"/);
