...
- 计划页使用“今天／明天／已完成”紧凑入口、独立“待安排”和统一清单；日历从右上角进入，支持“月历／周历”显示方式切换。固定 6 周月格每个日期只显示第一条安排的前 4 个字，不换行、不显示省略号或剩余数量；周历使用 7 天日期条联动选中日安排，并支持按钮或横向滑动换周。右上角显示完整年月并可按年份选择月份；月格不常驻重复的当日议程，点击日期后从底部打开当天全部安排。
