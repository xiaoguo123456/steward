...
- 首页标题下方提供“今天、时光、亲友、足迹、心情”可横向滚动并吸顶的页内标签导航；“今天”保留完整 Today 聚合，“时光”提供照片时间线、独立月历查找、1～9 张系统选图、手工编辑、AI 文案 Candidate 确认和详情查看的原生交互原型，“心情”已接正式心情日记。“亲友”在开发构建中提供明确标记的个人关系 CRM 界面预览，覆盖搜索、关系筛选、今天要关心、最近联系、全部亲友和人物详情；生产构建仍显示准备状态，不读取通讯录、不保存人物、不调用 AI。“时光”的演示数据与新增结果只保留在 App 进程内，不上传图片、不调用模型、不写正式接口；“足迹”继续展示不读取权限或用户资料的准备状态。
