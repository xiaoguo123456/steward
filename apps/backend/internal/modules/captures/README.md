...
- 回答追问不修改旧候选，而是推进 revision 重新解析；新 revision 只复制原始素材，
  历次“问题—回答”按 Question revision 正序重建并成对传给 Provider。
- Capture 追问不设置固定轮数上限；已经回答的信息不得重复追问，只有新的真实信息缺口才能继续追问。
...
AI 新记录项必须保留完整字段，Record 的同批引用在写候选时映射为 candidate_id；预先分配引用，支持前向依赖。引用与字段校验失败必须结束 Operation，不能保存不完整成功候选。历史空字段快照读取时暴露可恢复失败，保留原始输入，不补造字段。
