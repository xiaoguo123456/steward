-- +goose Up
-- 阿里云 PostgreSQL 16 暴露的系统角色名是 pg_rds_superuser。
-- 00030 使用旧角色名识别环境时会跳过，本迁移补齐数据库连接与对象权限隔离。
...
DECLARE
    ...
    admin_role text;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname IN ('pg_rds_superuser', 'rds_superuser')) THEN
        RETURN;
    END IF;
