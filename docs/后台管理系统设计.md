...
`STEWARD_ADMIN_PASSWORD_HASH` 使用 Argon2id。配置中不保存明文密码。
...
所有写请求同时校验 CSRF Token 和 Origin。登录接口按 IP 和用户名限流。
...
- 每小时更新当日 User Usage、AI Cost 和 Dashboard 数据。
- 每天按报表时区生成稳定日报。
- 聚合任务幂等，同一日期可安全重跑。
