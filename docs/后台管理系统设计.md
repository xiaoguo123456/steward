...
价格配置包含 Provider、模型、Usage Unit、单位大小、USD 单价和生效时间。修改价格时新增版本，已使用的历史价格不覆盖。人民币报价按有来源和日期的固定汇率折算 USD，并记录官方原价及来源；普通输入计费数量为总输入减去缓存输入。首个配置及补算流程见 [Qwen3.8 价格配置与成本验收](reviews/Qwen3.8价格配置与成本验收-2026-09-08.md)。
