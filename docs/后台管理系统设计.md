...
价格配置包含 Provider、模型、Usage Unit、单位大小、CNY 单价和生效时间。修改价格时新增版本，已使用的历史价格不覆盖。人民币官方报价直接保存，并记录来源；历史美元数据通过向前迁移折算为人民币，保留旧列用于兼容回滚；普通输入计费数量为总输入减去缓存输入。首个配置及补算流程见 [Qwen3.8 价格配置与成本验收](reviews/Qwen3.8价格配置与成本验收-2026-09-08.md)。
...
```text
cost = quantity / unit_size * unit_price_cny
```
...
unit_price_cny  numeric NULL
amount_cny      numeric NULL
