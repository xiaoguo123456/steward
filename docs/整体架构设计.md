...
- 心情日记正式实现仍挂在 `today` 路由的 `mood` 页内视图，首页列表、独立日历和花园由 `features/mood-journal` 维护；`mood-journal/new`、`mood-journal/[id]`、`mood-journal/search`、`mood-journal/calendar` 与 `mood-journal/garden` 分别承载新建、详情、搜索、日期查找和花园／回望，并在书写和 AI 授权流程中隐藏通用 AI 悬浮入口。日历页复用正式日期投影，只保留自身选中月份和日期，不创建第二份日记事实。它不增加第五个底部 Tab，也不接入页级 Pager；本地 Store 只保存当前日期、滚动位置和未同步草稿引用，日记正文与权威时间均来自正式 Query 或加密本地草稿。
