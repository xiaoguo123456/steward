...
sequenceDiagram
    ...
    M->>M: 对话内追加保存结果，显示全局紧凑 10 秒 UndoSnackbar
```
...
移动端 `UndoSnackbar` 位于全局层，按内容收缩并限制最大宽度 420dp；文字可随动态字号换行，撤销按钮保持至少 44×44dp，位置避开安全区和底部一级导航。10 秒窗口和 Activity 版本冲突语义不因视觉收紧而变化。
