...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／亲友／足迹／心情”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动默认 `today`，App Shell 保活期间可以保留选择。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片、人物筛选和地图控制的横向或连续手势冲突。
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API；“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情。时光原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态；“亲友”允许在 `__DEV__` 构建中渲染明确标记的非权威视觉 Fixture，生产构建仍回到准备状态，不定义网络 DTO、不调用 Provider、不持久化状态；“足迹”继续保持无权限、无数据读取、无接口调用的准备状态。正式实现亲友前必须通过产品决策与 ADR 引入 Person／Relation／Interaction／CareReminder 边界，完成 OpenAPI、生成 Client、Go Domain、用户隔离、幂等、来源、删除保留、联系人最小权限和 AI Candidate／Proposal 设计；重要日期继续复用 Event，不复制权威日期状态。
