...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／音乐／灵感／心情”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动默认 `today`，App Shell 保活期间可以保留选择。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片、来源浏览和音频控制的横向手势冲突。
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API，“灵感”复用正式 Notes Query；“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情；“音乐”保持无权限、无数据读取、无接口调用的准备状态。时光原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态。亲友人物关系预览保留为开发构建中的独立页面，不再占用首页顶部标签；生产构建仍不定义网络 DTO、不调用 Provider、不持久化状态。正式实现亲友前必须通过产品决策与 ADR 引入 Person／Relation／Interaction／CareReminder 边界，完成 OpenAPI、生成 Client、Go Domain、用户隔离、幂等、来源、删除保留、联系人最小权限和 AI Candidate／Proposal 设计；重要日期继续复用 Event，不复制权威日期状态。
- 时光原型复用已安装的 `expo-image-picker` 触发系统照片选择器，限制 1～9 张图片；图库选择不在页面打开时预申请权限，不读取未选择媒体。拍照能力若后续开放，只在用户点击后申请相机权限。Android Activity 被系统回收后的选图恢复、正式上传队列和跨设备同步属于正式媒体链路范围，不由原型 Store 冒充。
- `MemoriesPrototypeProvider` 只用于跨首页、日历、编辑和详情保留本地展示状态，类型归属移动端视图模型。正式时光实体、媒体引用、来源、版本、删除状态、AI Proposal 和幂等键必须先进入 OpenAPI 与 Go Domain；不得把本地 `MemoryMoment` 直接提升为网络 DTO，也不得与“长期记忆”领域的 `memory_items` 混用。
...
- “灵感”由 `features/inspiration` 的确定性选择器按当地日期从沉淀至少 7 天的未删除笔记中选择一条，并从内置通用题库稳定轮换一个主问题和三个候选方向。首页发现页同时展示这些真实来源与通用问题，不增加网络 DTO、模型调用或客户端权威状态。
- 问题和“继续想想”通过进程内一次性草稿 Store 把 `surface`、开场问题、来源标签和首个 Turn 上下文交给根 Stack 的既有 `/ai` 路由，避免把私人标题、正文或用户回复编码到 URL。普通 `/ai` 入口继续渲染 `AIConversationSheet`；`surface=inspiration` 的一次性上下文使同一对话实现渲染为全屏原生页面，从视觉上覆盖底部 Tab，但不复制 Thread、Message、流式响应、媒体 Capture 或 Proposal 状态。进程中断时这份上下文可以丢失，不作为业务数据恢复。
- 全屏灵感对话初始化为 `fresh` 会话，打开时不读取或创建当天默认 Thread。用户首次发送后才以 `force_new=true` 创建 Thread，并把页面上下文拼入首个 Turn；移动端展示时只呈现用户实际回复，不显示内部上下文。后续 Turn 与普通 Assistant 完全一致，页面上下文不得覆盖用户当前明确表达。
- 首页选中灵感标签时不渲染全局 `AIAssistantFab`，全屏灵感对话也不挂载底部一级导航、中央 Capture 或悬浮入口；切回其他首页标签后按原规则恢复悬浮入口。该条件只影响客户端展示，不改变 Capture Questions Query 或 Assistant 状态。
- 灵感对话的“整理成笔记”把用户主动触发的对话文本以 `origin=assistant` 送入统一 Capture，生成 Note Candidate 后进入既有 Processing／Confirmation；“生成待办建议”继续提交 Assistant Turn 并只允许产生 `task_create` Proposal。两条路径都复用正式用户隔离、来源、幂等和确认状态机，不允许客户端直接写 Note 或 Task。
- 后续多来源灵感由服务端 `Inspiration Application` 负责确定性候选筛选、用户隔离、来源版本校验、去重和每日批次幂等；AI Runtime 只生成 `inspiration_card_batch@v1` 的标题、联系说明和可选建议。卡片 Snapshot 保存 `SourceRef` 与来源版本，不复制高敏正文；来源失效后标记 stale。移动端不得自行调用模型、用 Embedding 决定权限或把反馈静默升级为 Memory。
...
日记草稿只为中断恢复服务，不是正式事实。移动端使用项目统一 SQLite 草稿封装，并通过平台密钥保护正文；草稿键包含当前用户、设备、目标日记或临时 ID，登出、账号删除或正式保存后按策略清理。生物识别／设备凭据只负责解锁本地页面和草稿，不能替代服务端会话、RLS 或 API 授权；App 进入后台时必须遮挡任务切换预览中的正文。
...
| 图标 | Hugeicons Free Stroke Rounded 为唯一 App 图标字形；统一从 `packages/ui/Icon` 语义映射后导出 |
...
Expo Router 使用文件路由并支持深链，适合把通知与来源链接稳定映射到页面；TanStack Query 官方提供 React Native 的网络、焦点和在线状态接入方式。Tamagui 负责可主题化基础组件和跨平台 Token；Expo UI 提供分段控件等标准输入，滑杆采用 Expo 官方支持的 `@react-native-community/slider`，两者都必须经 `packages/ui` 品牌适配后使用。Hugeicons Free Stroke Rounded 提供 App 唯一的图标语言，通过 `react-native-svg` 渲染；所有字形由 `packages/ui/Icon` 完成稳定语义键映射、1.8 默认线宽与光学校准。Feature 只能依赖品牌封装，不直接导入底层 UI 或图标包，也不得在页面中写裸色值或自行维护 SVG；第三方图标名称不进入业务数据和网络契约。参考：[Expo Router](https://docs.expo.dev/router/introduction/)、[TanStack Query React Native](https://tanstack.com/query/latest/docs/framework/react/react-native)、[Tamagui Expo Guide](https://tamagui.dev/docs/guides/expo)、[Expo UI](https://docs.expo.dev/versions/latest/sdk/ui/)、[React Native Community Slider](https://docs.expo.dev/versions/latest/sdk/slider/)、[Hugeicons React Native](https://hugeicons.com/docs)。
...
法律文本、信息清单和帮助内容复用 `apps/mobile` 的 Expo 静态 Web 输出，不新增 WebView 或独立部署服务。纯内容页面放在 `apps/mobile/public/legal/*/index.html` 与 `apps/mobile/public/support/index.html`，导出时原样复制到 `dist`；它们不进入 App Root Provider，不读取 SecureStore，不复用用户 Query Cache，也不调用用户 API。`/legal/*` 与 `/support` 必须免登录、关闭 JavaScript 后仍可读；App 使用 `expo-web-browser` 或系统浏览器打开当前环境的 HTTPS URL，不能用 App 私有深链冒充应用商店可访问 URL。
...
账号删除说明可以使用静态页面，但 Web 删除请求属于正式业务能力：必须新增中立 OpenAPI，使用手机号验证码与单用途 reauth，经 Auth／Retention Domain 执行幂等、限流、会话撤销和删除状态机。到该阶段再评估用隔离的公开 Web 入口或 `apps/public-web` 承载生成 Client 与表单逻辑，同时保持 `/legal/account-deletion` URL 不变。H5 不得手写平行 DTO、直接调用内部管理接口或用静态表单声称已经受理。完整载体矩阵、固定 URL、版本规则和开发提醒见《H5 与公开页面边界》。

### 6.2.2 功能型 H5 与远程更新

当前架构只包含公开静态 H5，不包含登录态 Web 容器。移动端已按 `ADR-028` 集成 `expo-updates`，但本地无凭证时默认关闭；正式 APK 构建要求显式注入 Expo Project ID 和验签证书，缺失时直接失败。新增功能仍先按《H5 与公开页面边界》记录载体决策：只有读为主、轻交互、在线可用且不依赖原生权限或核心敏感写入的模块进入功能型 H5 候选。
...
当前正式登录只包含短信验证码。平台电话／验证码 Autofill 只负责减少输入，不形成认证事实；服务端仍必须校验验证码。运营商一键登录接入前必须新增中立 OpenAPI、服务端 Adapter、capability、隐私授权和三网真机测试，不能复用短信 `code` 字段伪装运营商 Token。
...
| 模块 | 主要接口 |
|---|---|
| TaskLists | `GET/POST /task-lists`、`PATCH /task-lists/{id}`（编辑、归档和恢复）；保留的 DELETE 接口不进入移动端产品路径 |
...
| Projects | `GET/POST /projects`、`GET/PATCH /projects/{id}`、`POST /projects/{id}/transition`、`POST /projects/{id}/delete`（必须带关联 Object 处理策略）；通用 Project 由 Capture Confirmation 创建，`POST /projects` 只供字段完全可见且用户明确确认的确定性专用表单使用，例如 `project_kind=trip` 的行程 |
| Notes | `GET/POST /notes`、`POST /notes/polish`、`GET/PATCH/DELETE /notes/{id}`、`POST /notes/{id}/convert-to-task`；正文使用 `NoteContent` 判别联合，当前普通 Note 创建只接受 `plain_text` |
| Mood Journal | `GET/POST /mood-journal/entries`、`GET/PATCH/DELETE /mood-journal/entries/{id}`、`GET /mood-journal/calendar`、`GET /mood-journal/statistics`；正文写入复用 Note Command 并固定接受 `blocks_v1`，普通 Notes API 不接受 `mood_journal` kind |
...
| POST | `/mood-journal/follow-up-generations` | 对用户明确选择的单篇日记生成一个可跳过追问，返回 `202 + operation_id`；不修改原文 |
| POST | `/mood-journal/reflection-generations` | 对用户选定范围和条目生成带来源的周／月回望，返回 `202 + operation_id` |
| POST | `/mood-journal/reflections/{reflection_id}/confirm` | 用户确认保存回望；重验来源版本和逐篇 AI 排除状态，任一失效则拒绝旧候选 |
...
移动端月历将 42 个日期桶用于完整月格和点击日期后的当日安排面板，周历则请求选中日期所在周的 7 个日期桶。日期格只摘要排序后的第一条安排前 4 个字，不渲染同日剩余数量；周历日期条展示每日计数，下方直接读取同一响应中选中日期桶，不另外请求平行议程，也不保存为新的领域状态。
...
- Task 所有状态转换。
- Task transition API 的 `start/stop/complete/cancel/reopen` 全部合法与非法来源状态；时间字段 PATCH 不改变 `todo/doing/done/cancelled` 状态，只影响“随时可做”等查询筛选。
...
- Calendar 42 天月格日期桶包含 Task、Event、重要日期和 Project 节点，且同一来源不重复、无日期 Task 与 Record timestamp 不进入。
- Project 进度和状态恢复。
- Candidate 依赖闭包。
- 五类实体重复候选规则。
- Today 收录、分组和稳定排序。
...
- 功能设计和发布记录必须标明使用 H5、OTA 还是原生版本；服务端 capability 只决定已安装客户端能否启用兼容功能，不能补齐旧二进制缺少的原生桥接、权限或 runtime。
...
| ADR-014 | 资源和账号删除状态机 |
| ADR-015 | Tamagui 品牌组件封装与 Hugeicons 单一图标语义适配层 |
