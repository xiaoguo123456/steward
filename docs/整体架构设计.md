...
当前架构只包含公开静态 H5，不包含登录态 Web 容器，也未接入 `expo-updates`。新增功能先按《H5 与公开页面边界》记录载体决策：只有读为主、轻交互、在线可用且不依赖原生权限或核心敏感写入的模块进入功能型 H5 候选。
...
若后续采用 `expo-updates`，必须先建立 ADR，明确代码签名、test／production 通道隔离、runtime version、灰度、监控和回滚。OTA 只承载与已安装原生 runtime 兼容的 JavaScript、样式与资源；原生 SDK、依赖、权限、后台能力和容器变化重新构建并走应用商店发布。H5、OTA 与 Feature Flag 均不能绕过审核或改变服务端权威状态机。
...
- 当前不得把 OTA 作为可用能力写入排期或用户承诺；完成 `expo-updates` ADR、原生接入、签名、通道隔离和回滚演练后才能启用。
