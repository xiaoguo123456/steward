...
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API，“灵感”复用正式 Notes Query，“时光”使用生成的 Memory Moments Query；`memories/calendar`、`memories/new` 与 `memories/[id]` 分别承载日历查找、发布和只读详情。“亲友”使用生成的 People Query 和原生 `people/*` 路由；Go `relationships` 模块拥有 Person、PersonInteraction 与 `event_people` 关联，人物事件继续由 Objects 模块创建正式 Event。三张表均启用 RLS，跨表外键携带同一 `user_id`；创建和删除使用幂等键，人物修改使用乐观锁。人物删除级联互动和关联，但不删除 Event。首版不读取联系人，也不向 AI Provider 发送人物资料。音乐不进入当前首页导航，也没有现行移动端领域或 Provider 链路。
...
| 亲友 | `features/relationships`、`app/people` | `modules/relationships`、`modules/objects` | `people`、`events` | 首版无异步任务；AI 人物访问默认关闭 |
