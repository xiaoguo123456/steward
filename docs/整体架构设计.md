...
- `features/workouts` 使用 Expo Location 进行仅前台的 GPS 采集，使用 MapLibre React Native 加载 CDN 上的 PMTiles 地图样式。客户端确定性过滤低精度、漂移、瞬移和断档坐标，按路线段计算距离、平均配速或平均速度；暂停、确认退出和页面卸载时停止订阅。原始坐标只存在于当前会话，不上传、不持久化；总结页经用户确认后仅将时长、距离等结果通过既有 Tracker / Record 契约保存。当前不提供后台定位、会话恢复或计步器能力，健走不展示或保存没有可靠来源的步数。
