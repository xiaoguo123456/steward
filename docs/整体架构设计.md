...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／亲友／灵感／心情”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动默认 `today`，App Shell 保活期间可以保留选择。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片、人物筛选和来源浏览的横向或连续手势冲突。
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API，“灵感”复用正式 Notes Query；“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情；“亲友”在 `__DEV__` 构建中渲染非权威固定视觉 Fixture，生产构建只显示准备状态，不定义网络 DTO、不调用 Provider、不持久化状态。开发 Fixture 的非权威边界由 `__DEV__` 门禁、文档与源代码测试保证，不依赖用户界面的研发提示，也不暴露尚未承接正式 Command 的新增入口。时光原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态。正式实现亲友前必须通过产品决策与 ADR 引入 Person／Relation／Interaction／CareReminder 边界，完成 OpenAPI、生成 Client、Go Domain、用户隔离、幂等、来源、删除保留、联系人最小权限和 AI Candidate／Proposal 设计；重要日期继续复用 Event，不复制权威日期状态。音乐不进入当前首页导航，也没有现行移动端领域或 Provider 链路。
- 时光原型复用已安装的 `expo-image-picker` 触发系统照片选择器，限制 1～9 张图片；首页或月历的明确用户点击可以通过 `memories/new` 的纯展示路由参数携带“立即选图”和预填日期，编辑页只消费该参数一次，不把它保存为领域状态。图库选择不在页面打开时预申请权限，不读取未选择媒体。拍照能力若后续开放，只在用户点击后申请相机权限。Android Activity 被系统回收后的选图恢复、正式上传队列和跨设备同步属于正式媒体链路范围，不由原型 Store 冒充。
