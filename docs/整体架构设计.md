...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／音乐／足迹／心情”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动默认 `today`，App Shell 保活期间可以保留选择。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片、地图和音频控制的横向手势冲突。
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API；“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情。时光原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态；“音乐、足迹”继续保持无权限、无数据读取、无接口调用的准备状态。其余分区正式实现前分别完成媒体、音频、地图／定位的契约、Provider、权限、删除保留和 AI Candidate／Proposal 设计，不能把前端展示模型提升为网络 DTO，也不能在客户端直连模型或持久化权威状态。
