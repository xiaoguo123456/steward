...
- 当前只有“今天”连接正式 Today Query。“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情；原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态。“音乐、足迹、心情”继续保持无权限、无数据读取、无接口调用的准备状态。正式实现前分别完成媒体、音频、地图／定位、私密正文的契约、Provider、权限、删除保留和 AI Candidate／Proposal 设计，不能把前端展示模型提升为网络 DTO，也不能在客户端直连模型或持久化权威状态。
...
| Notes | `GET/POST /notes`、`POST /notes/polish`、`GET/PATCH/DELETE /notes/{id}`、`POST /notes/{id}/convert-to-task` |
...
`POST /notes/polish` 是不落业务库的同步单次结构化生成：只接收当前草稿标题与正文，先校验 AI 建议开关与长度，再在事务外调用 Provider，并用项目 JSON Schema 校验原始结果。已有标题由 Go 确定性覆盖回原值；结果返回编辑器，不直接创建或更新 Note。成功调用写入不含正文的 AI Action 审计并返回其 ID；用户最终创建 Note 时回传该 ID，服务端校验当前用户、功能策略和成功状态后写入 `provenance_refs`。
