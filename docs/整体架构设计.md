...
- 当前只有“今天”连接正式 Today Query。其他四项只渲染无权限、无数据读取、无接口调用的准备状态；正式实现前分别完成媒体、音频、地图／定位、私密正文的契约、Provider、权限、删除保留和 AI Candidate／Proposal 设计，不能把前端展示模型提升为网络 DTO，也不能在客户端直连模型或持久化权威状态。
