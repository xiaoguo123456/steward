...
- 当前“今天”连接正式 Today Query，“心情”连接正式心情日记 API，“灵感”复用正式 Notes Query；“时光”在 `features/memories` 提供原生交互原型，并通过 `memories/calendar`、`memories/new` 与 `memories/[id]` 承载日历查找、编辑和详情；“亲友”在 `__DEV__` 构建中渲染明确标记的非权威视觉 Fixture，生产构建只显示准备状态，不定义网络 DTO、不调用 Provider、不持久化状态。时光原型数据、AI Candidate 和新增结果只保留在 App 进程内，不上传媒体、不调用模型、不写正式领域状态。正式实现亲友前必须通过产品决策与 ADR 引入 Person／Relation／Interaction／CareReminder 边界，完成 OpenAPI、生成 Client、Go Domain、用户隔离、幂等、来源、删除保留、联系人最小权限和 AI Candidate／Proposal 设计；重要日期继续复用 Event，不复制权威日期状态。音乐不进入当前首页导航，也没有现行移动端领域或 Provider 链路。
