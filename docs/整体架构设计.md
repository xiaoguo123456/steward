...
周历时间轴以响应的用户时区把绝对时刻格式化到分钟，并基于 Event／Task 原始起止时间生成可丢弃的视觉片段；跨午夜拆列、最小时高和重叠分栏都只是客户端布局状态，不成为网络字段或本地权威事实。日程详情隐藏 IANA 时区属性行不代表删除语义字段，读取、编辑、提醒和跨时区计算继续使用 Event `timezone`。
