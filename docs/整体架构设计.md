...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／心情／亲友”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动首次挂载默认 `today`，从时光、心情或亲友的二级／更深路由原路返回时保留当前标签。跨栈流程需要指定日记日期或目标列表时可以携带一次性 `homeTab/date` 展示参数，首页只在参数存在时消费并立即清理。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片和人物筛选的横向或连续手势冲突。
