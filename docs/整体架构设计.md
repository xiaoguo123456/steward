...
该树只展示当前主要入口，以仓库文件为准。全局搜索、Activity 历史、原生隐私控制、账号删除执行、数据导出和“最近删除”当前没有对应移动端路由；其中数据导出和“最近删除”不属于本版范围。公开法律与支持入口已收纳在 `/me`，账号删除仍只有公开说明，没有原生执行与 Web 申请链路。不得把产品设计中的目标路由清单写成已经存在的文件。
...
账号删除说明已使用静态页面，但 Web 删除请求属于尚未实现的正式业务能力：必须新增中立 OpenAPI，使用手机号验证码与单用途 reauth，经 Auth／Retention Domain 执行幂等、限流、会话撤销和删除状态机。到该阶段再评估用隔离的公开 Web 入口或 `apps/public-web` 承载生成 Client 与表单逻辑，同时保持 `/legal/account-deletion` URL 不变。当前页面刻意不提供表单，也不声称已受理。H5 不得手写平行 DTO、直接调用内部管理接口或用静态表单声称已经受理。完整载体矩阵、固定 URL、版本规则和开发提醒见《H5 与公开页面边界》。
...
当前移动端尚未实现这套按账号隔离的持久化仓库，文字、图片和录音只在当前输入页会话中保留；有内容退出时明确提示放弃，不能把返回操作写成“保存草稿”。在本地媒体复制、容量清理、账号隔离、退出登录选择和离线恢复一起完成前，不得声称 CAP-007 已实现。
