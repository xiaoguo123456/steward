...
- 记账场景由 `features/[slug]` 路由承载。手工账单、月度统计和用户确认后的结果复用内置 `ledger` Tracker／Record；拍照／相册入口仍只模拟来源选择和识别 Candidate，不申请真实媒体权限、不上传图片，也不调用 OCR。正式识别必须先进入统一 Capture 媒体链路，用户确认后再由 Go Domain 写入；账单图片不得写入日志、埋点或普通测试快照，本地 `LedgerEntry` 只是视图模型。
- 重要日场景由 `features/[slug]` 路由承载，列表和新增均使用生成 Client。生日、纪念日、到期日和其他映射到正式 `important_date_kind`，不创建新的 Object Domain；所有内容保存为 `event_kind=important_date` 的全天 Event，并由 Go Domain 处理年度重复、2 月 29 日、当地 09:00 提醒与日历投影。当前不申请通知、联系人或系统日历权限，服务端 Reminder 不等于设备推送已接入。
...
该树只展示当前主要入口，以仓库文件为准。全局搜索、Activity 历史、隐私控制、账号删除、数据导出和“最近删除”当前没有对应移动端路由；其中数据导出和“最近删除”不属于本版范围，账号删除与公开法律入口是发布前缺口。不得把产品设计中的目标路由清单写成已经存在的文件。
...
当前决策范围只允许公开静态 H5，不包含登录态 Web 容器；静态 H5 本身仍待实现。移动端已按 `ADR-028` 集成 `expo-updates`，但本地无凭证时默认关闭；正式 APK 构建要求显式注入 Expo Project ID 和验签证书，缺失时直接失败。新增功能仍先按《H5 与公开页面边界》记录载体决策：只有读为主、轻交互、在线可用且不依赖原生权限或核心敏感写入的模块进入功能型 H5 候选。
...
| 跨页面临时状态 | React Context 或最小一次性 Store | Assistant 首轮上下文、原型会话状态 |
| 小型可恢复草稿 | Secure Store | 当前心情日记草稿 |
| 凭证 | Secure Store | Access / Refresh Token |
| 原型数据 | 进程内 Context | 时光演示条目；重启后恢复 Fixture |
...
独立 Notifications、Scheduler、Exports、Retention 与自然语言 Search Answer 模块当前不存在，不能据此假定通知投递、导出或账号删除已经落地。新增模块时必须同时补目录、数据所有权、契约、迁移、测试和本节。
...
当前进入 Worker 的任务包括 Capture 解析、Assistant 回复、周复盘叙述生成、TaskList／Tracker 归档到期清理与 Admin 聚合。音频转写、独立 OCR 流水线、Search Embedding、推送、导出和账号删除任务尚未接入；这些目标能力在实现前必须补正式 JobArgs、队列、状态、重试和验收。
...
当前 `processed_jobs` 以业务幂等键记录已处理结果，HTTP 幂等使用迁移中已有的 `idempotency_keys`。下面的 `system_job_runs`、`http_idempotency_records` 与敏感响应重放结构属于未来高影响操作的目标设计，当前迁移中不存在。
...
当前 Auth 契约见 `packages/contracts/openapi/paths/auth.yaml`，用户资料与换绑契约见 `packages/contracts/openapi/paths/users.yaml`：
...
当前没有 `/me/bootstrap`、Onboarding 草稿、`logout-all` 或高影响操作 reauth API；相关能力不能在 App 中以本地状态或手写请求代替。需要新增时，先完成产品状态、OpenAPI、生成代码、服务端行为和跨端测试。
...
| Account deletion | 发布前缺口 | App 与公开 Web 删除链路尚未进入 Public OpenAPI；未来必须使用专用 reauth、幂等、限流和独立状态凭证 |
...
未来实现账号删除时，必须提交未过期、用途为 `account_delete` 的 `reauth_token`、明确确认字段和 `Idempotency-Key`。成功后返回 `202`、`deletion_request_id` 与高熵、仅显示一次的 `status_token`，立即撤销正常 Session；App 把 token 放入安全存储。状态 GET 在 OpenAPI 中使用独立 `DeletionStatusToken` security scheme，经 `WithDeletionStatusTx` 只返回无用户内容镜像；该 token 不被任何普通 API 接受，接口按 request ID、token 摘要和 IP 限流。失败且请求未被接受时不注销。该段是待实现契约约束，不表示接口已经存在。
...
- 当前受保护请求验证 Access Token，并按当前用户隔离资源；`/v1/auth/logout` 注销当前 Refresh Token。全设备注销、再次验证与账号删除接受尚未进入 Public OpenAPI，不能声称已有 Session 全量撤销或删除重放机制。
...
| 删除账号（待实现） | 否 | 全部会话、业务数据、媒体、派生产物、提醒、通知、导出包、缓存和 Provider 侧可删除数据 | 不含业务内容的删除请求审计与备份过期边界 |
...
账号删除的目标流程如下。Public OpenAPI、App 内入口和公开 Web 请求链路当前均未实现，正式编码前还必须按 [H5 与公开页面边界](./H5与公开页面边界.md) 完成合规门禁：
...
- 登录、Refresh、当前 Session Logout 与失效 Token 路径；全设备注销、reauth 和账号删除在契约落地后再加入同层测试。
...
- [ ] 资产上传和查看使用短期授权。
- [ ] 已承诺的 TaskList 72 小时归档恢复、Tracker 30 天归档恢复和 Activity 即时撤销可端到端验证；Capture 自动清理、账号删除或未来导出只有在正式实现后才能加入验收声明。
