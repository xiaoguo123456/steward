...
- 首页的 4×2 生活场景入口由 `today` 页面编排，场景页面归属 `features/[slug]` 二级路由，不增加第五个一级模块。运动使用静态优先的 `features/exercise/*` 嵌套路由承载首页、准备、进行中、总结和历史页面，其余轻量场景仍可使用 `features/[slug]`；静态路由只细化页面流程，不改变一级导航或领域所有权。运动、番茄钟与记账组合内置 Tracker / Record 作为统一数据底座，但由各自场景展示和录入，不投影到打卡首页；打卡首页只读取自定义 Tracker，并由 Go Domain 按用户时区、频率和当天 Record 确定 `due_today`。食谱与购物组合 Note / TaskList / Task，重要日投影 Event 与日期字段，复盘读取 Review，“更多”只聚合二级导航。行程使用静态优先的 `trips`、`trips/new` 与 `trips/[id]` 路由承载总览、新建和详情，作为 Project 及其关联 Event、Task、Note、附件和 Relation 的聚合读模型；专用新建页的手工路径通过生成的 API Client 调用既有 Project Command，AI 路径复用统一 Capture，在 `capture-parse@v2` 中生成带 `project_kind=trip`、目的地和起止日期的 Project Candidate，确认事务再调用同一个 Project Command。最终 Project 仍以 description 首行／余下内容保存目的地和注意事项。它不复用 `calendar` 的呈现层，也不新增 Trip 领域、DTO 或平行数据表；场景页不得让 AI 直接写库。
...
`origin=project_manager` 与 `suggested_project_id` 只属于 Capture UI Context，不进入 Project 创建 Command，也不能单独作为强制类型字段传给模型。AI 仍依据用户输入事实生成 Task／Event／Project 等 Candidate；只有 active Candidate Snapshot 中被用户选择并确认的 Project Candidate，才由 Capture Confirmation 事务调用 Project Repository 创建。`trips/new` 同时提供确定性表单和“AI 创建”：前者通过生成的 `/projects` Client 固定创建 `project_kind=trip`；后者由用户显式点选行程入口，客户端把“创建行程”作为可见意图与文字／语音／图片素材一并提交，模型提取名称、目的地、起止日期和注意事项，确认后再创建。入口上下文仍不能代替候选内容校验或用户确认。
