...
后台手机号身份隔离、验证码消费、名单权限和会话版本规则见
[后台手机号认证](reviews/后台手机号认证验收-2026-09-08.md)。Admin OpenAPI 同步生成
Go、TypeScript 与 Zod；原生 App 无对应接口适配变化。
...
Go 后端、可替换 AI Runtime、Assistant、意图识别、受控工具调用和用户长期记忆的落地细节见 `后端与AI开发指南.md`。当前 Assistant 固定使用 Eino v0.9.19 单 Agent，但业务只依赖项目自有 `OrchestrationEngine`；该指南不得覆盖本文的模块边界、契约治理、事务、用户隔离和“AI 不直接写库”规则。
...
- `calendar` 是从计划页右上角进入的独立二级路由，不是底部 Tab、模态弹窗或新的 Object Domain。月历请求 42 天并渲染固定 6 周月格，日期格只显示第一条内容的前 4 个字，不显示剩余数量；周历按选中日期请求周日至周六 7 天，移动端用纯展示模型把响应中的定时 Event 和 Task 切成七列时间轴片段，处理跨午夜裁切与重叠分栏，全天事实单列展示。日期桶和响应顺序仍由 Go 确定，客户端时间轴布局不得回写或改变事实归属。“月历／周历”切换只改变请求范围与客户端展示，不创建新的领域状态。页面按日期聚合已确认 Task、Event、重要日期与 Project 日期节点，复用计划详情层的 Project scope、时区和实体详情路由；返回时保留计划页滚动与选择状态。
...
- `today` 路由内部使用 `features/home/home-top-tabs` 承载“今天／时光／心情／亲友”顶部标签。标签选择只是 React 本地展示状态，不增加底部 Tab、路由栈、网络字段或 Domain 状态；冷启动首次挂载默认 `today`，从时光、心情或亲友的二级／更深路由原路返回时保留当前标签。跨栈流程只可携带一次性 `homeTab` 展示参数指定目标分区，首页消费后立即清理；不接受为心情新建预选日期的 `date` 参数。标签栏自身允许横向滚动并随纵向内容吸顶，但内容不接入页级 Pager，避免与照片和人物筛选的横向或连续手势冲突。
...
- 心情日记正式实现仍挂在 `today` 路由的 `mood` 页内视图，首页列表、独立日历和回望由 `features/mood-journal` 维护；`mood-journal/new`、`mood-journal/[id]`、`mood-journal/search`、`mood-journal/calendar` 与 `mood-journal/reflections/[period]` 分别承载新建、详情、搜索、日期查找和周／月回望，并在书写和 AI 授权流程中隐藏通用 AI 悬浮入口。首页只保留“本月回望”入口，回望路由在同一页面组合确定性视觉轨迹、统计、感受词、逐篇选择和可选 AI 结果，不再维护独立花园页面或第二入口。心情和时光的日期查找复用 `features/calendar` 中无业务请求的月份导航与 6×7 日期格组件；各领域自行提供日期计数、读屏文案、配色和下方内容列表，公共组件不得读取日记或时光 API。心情日历只触发月份／日期查询和既有日记详情导航，不发起新建日记路由；`mood-journal/new` 不消费日期路由参数，创建请求在用户实际保存时生成 `occurred_at`。日历页只保留自身选中月份和日期，不创建第二份日记事实。它不增加第五个底部 Tab，也不接入页级 Pager；本地 Store 只保存当前日期、滚动位置和未同步草稿引用，日记正文与权威时间均来自正式 Query 或加密本地草稿。
...
- App Shell 在首页、计划、笔记、打卡和设置 Stack 上方挂载同一个常驻 `AIAssistantFab`，点击后统一进入 `AIConversationSheet`。入口聚合当前用户的 open Capture Question、pending Assistant Proposal 和当前 App 会话内的 active Capture 引用数量，不复制问题正文或 Candidate 到全局 Store；数量为零时只隐藏角标，入口本身继续可见。中央 `capture/new` 不读取待答数量，也不显示角标。`AIAssistantFab` 的停靠侧与归一化纵向位置属于纯展示会话状态，由轻量 Store 跨页面共享，不进入 API、Capture Query 或业务状态机；拖动范围由页面布局计算并始终限制在安全区。
- `CaptureAssistantSessionProvider` 仅在当前 App 会话保存 `capture_id`、`operation_id`、入口上下文和最小展示摘要，用于关闭浮层后恢复；Operation、Capture、Question、Candidate 和 Confirm 状态仍以服务端 Query 为权威，Provider 不复制正文、不推演状态，也不成为第二份领域事实。进程重启后的恢复继续由按账号隔离的“最近输入”草稿／队列记录进入同一 `AIConversationSheet`。
- AI 对话输入中的纯文字继续走 Assistant Turn；一旦包含图片，移动端复用 `useImagePicker`、媒体上传授权与 `POST /captures`，以 `origin=assistant` 创建多模态 Capture。中央 Capture 提交也在创建成功后直接 replace 到 `/ai` 并打开 `AIConversationSheet`；处理中、澄清、候选编辑、确认保存与失败恢复由 `AssistantCaptureFlow` 在同一浮层按服务端状态追加。未上线的 `capture/processing`、`capture/confirm` 与 `assistant/pending` 路由及页面实现全部删除，不保留兼容入口。该分支不扩展 `CreateTurnRequest`，也不复制上传、解析或确认状态机。
- 载体决策为 React Native 原生核心流程：Capture 依赖本地加密草稿、媒体权限、上传恢复、动态状态和敏感写入确认，不迁移到 H5。本次路由与组件调整不新增公开页面、第三方处理者、个人信息类型或权限，也不改变单独同意和应用商店申报。
- 首页的 4×2 生活场景入口由 `today` 页面编排，场景页面归属 `features/[slug]` 二级路由，不增加第五个一级模块。运动、番茄钟与记账组合内置 Tracker / Record 作为统一数据底座，但由各自场景展示和录入，不投影到打卡首页；食谱与购物组合 Note / TaskList / Task，重要日投影 Event 与日期字段，复盘读取 Review。行程使用 `trips`、`trips/new`、`trips/new-item` 与 `trips/[id]`，作为 Project 及其关联 Event、Task、Note 和媒体引用的聚合读模型。`capture-parse@v5` 同时支持创建行程 Project Candidate、按目标版本更新已有实体，以及生成候选间或候选与已有实体之间的 Relation Candidate，并按时间正序理解已经发生的澄清问答。它不新增 Trip 领域、DTO 或平行数据表；场景页不得让 AI 直接写库。
- `features/workouts` 使用 Expo Location 进行仅前台的 GPS 采集，使用 MapLibre React Native 加载 CDN 上的 PMTiles 地图样式。客户端先以精度不差于 30 米的有效坐标建立首次 GPS fix，fix 前不启动计时和轨迹累计；开始后再确定性过滤低精度、漂移、瞬移和断档坐标，按路线段计算距离、平均配速或平均速度。暂停、确认退出和页面卸载时停止订阅。语音节点和播报文本由客户端确定性生成，首版使用 `expo-speech` 的设备 TTS，避免运动中依赖网络；未来品牌音色可由服务端调用语音模型生成并缓存到对象存储，但不得把模型密钥放进 App。原始坐标只存在于当前会话，不上传、不持久化；用户开启常亮后由 `expo-keep-awake` 在运动进行中页面持有并随页面卸载释放。总结页等待 Tracker / Record Command 成功后才离开，失败保留输入供重试。当前不提供后台定位、会话恢复或计步器能力，健走不展示或保存没有可靠来源的步数。
- 食谱使用静态优先的 `features/recipes/*` 嵌套路由承载本周菜单、发现、问卷、菜谱详情、烹饪和购物确认，`features/recipes` 模块只保存非权威的前端菜谱模型和场景状态。发现分类由服务端查询；客户端仅在无搜索词时使用会话种子对每个分类做稳定展示洗牌，下拉刷新更换种子，搜索继续使用服务端相关性顺序，随机展示不改变服务端数据和权威排序。新生成或批量调整的整周草稿在本周首页日期栏后、菜单内容前显示确认入口，确认成功前不显示生成购物清单操作。单道替换不得从发现页前 100 条内容中本地轮播，而是复用服务端确定性菜单建议取得已完成过敏原、忌口、餐次、菜品角色与营养筛选的候选；确认面板通过整周菜单接口原子保存，不增加逐格写入接口。时令食材的自然上市月份由离线模型调用生成并经 Schema 校验，再由确定性代码归并成春、夏、秋、冬四季。菜谱打标、来源餐次到平台 `meal_slots` 的校正和当前季节过滤均由确定性代码完成；来源分类保留在 `tags` 供审计，服务端按中国时区自动切季，线上请求不调用模型。本周菜单中的营养数值属于计划估算，只有用户确认进食记录后才能通过正式 Tracker / Record 契约形成实际摄入；两种口径不得复用同一状态字段。外部菜谱必须记录来源、作者、图片权利、授权范围和内容版本。个人收藏或补充说明继续复用 Note，确认后的采购需求继续复用 TaskList / Task；菜单采用、加入某餐和生成购物清单均由确定性服务校验并经用户确认。
- 专注场景使用稳定路由 `focus`。计时、暂停、休息和会话内想法由客户端确定性状态驱动，不安装后台任务或通知依赖；Today Task 引用来自正式 Query，用户保存后通过内置 `focus` Tracker 写入正式 Record。Task 完成与专注 Record 是两个 Command：用户明确选择“标记任务完成”时，客户端先携带 Task 版本执行正式更新，再等待 Record 保存；失败保留总结页，Record 失败后的重试不得重复完成 Task。后台恢复与跨设备计时仍未实现，本地 `FocusRecord` 只是视图模型。
...
- 重要日场景由 `features/[slug]` 路由承载，列表、新增、编辑、标记处理和删除均使用生成 Client。生日、纪念日、到期日和其他映射到正式 `important_date_kind`，不创建新的 Object Domain；所有内容保存为 `event_kind=important_date` 的全天 Event，并由 Go Domain 处理年度重复、2 月 29 日、当地 09:00 提醒、日历投影与列表排序。过期由 `days_until` 确定性派生，用户处理由 `important_date_handled_at` 独立持久化；已处理的一次性 Event 从活动重要日、Today 和提醒读模型退出，但保留在历史日历，删除仍走 Event 软删除。读模型先返回今天和未来条目，再返回未处理且已过期的一次性条目；移动端保留组内顺序，只将两类数据分开展示。当前不申请通知、联系人或系统日历权限，服务端 Reminder 不等于设备推送已接入。
...
- 四个一级路由各自保存滚动位置和查询缓存。计划页的时间范围与具体清单属于页内临时视图：用户从计划点击其他一级 Tab 时由 App Shell 发出纯前端重置信号，再次进入计划落到首页；打开任务详情、日历或 Capture 等二级路由不会触发该信号，返回时继续保留原计划上下文。打开 Capture 输入面板、AI 对话内确认或详情页时隐藏中央主操作，防止重复主操作和安全区遮挡。
...
| 心情日记 | `features/mood-journal` | `modules/objects`、`modules/moodjournal` | `objects`、`mood-journal` | 排版、追问与最多 31 篇的周／月回望使用受限同步 AI 候选接口；独立 Search Index 异步更新 |
...
日记草稿只为中断恢复服务，不是正式事实。当前移动端使用 `expo-secure-store` 保存按用户与目标日记隔离的小型草稿，正式保存后删除；登出与账号删除仍须按统一清理策略覆盖。若正文规模或跨草稿恢复需求超过 Secure Store 的适用边界，再设计加密的通用本地草稿仓库，不能直接把未加密正文落入 SQLite。当前版本不提供应用锁，不引入 `expo-local-authentication`，也不申请面容或指纹用途权限；私人数据继续依赖设备系统锁、Secure Store／SQLCipher、服务端会话、RLS 和高风险操作重新认证形成边界。
...
`POST /v1/mood-journal/entries/{entry_id}/follow-up` 在保存后由用户主动触发，只读取当前版本的单篇纯文本投影，返回一个带来源 ID、版本和 AI Action 的问题候选。`POST /v1/mood-journal/reflections` 接收日期范围与本次明确勾选的 1 至 31 个 ID；Go 在 Provider 调用前重新校验用户归属、范围和 `exclude_from_ai`，在输出后再校验所有观察与建议的来源只属于选择集合。两种结果都不自动落库，也不写 Note 或 Memory；门禁失败使用正式错误码，确定性统计保持可用。
...
`/me` 是设置聚合一级页；显示名称使用该页内底部面板编辑，不创建 `/settings/profile`。脱敏手机号与退出登录也直接位于 `/me`，只有双验证码换绑等独立安全流程进入 `/settings/phone`。“专注设置”和“应用锁”不在当前设置聚合中展示；番茄钟继续从首页场景入口进入。系统时区由 Mobile 在登录、启动和回到前台时通过现有 `PATCH /v1/me` 自动同步，不暴露普通设置入口。
...
公开 H5 已按以下结构落地。法律文本、信息清单和帮助内容复用 `apps/mobile` 的 Expo 静态 Web 输出，不新增独立部署服务。纯内容页面位于 `apps/mobile/public/legal/*/index.html` 与 `apps/mobile/public/support/index.html`，导出时原样复制到 `dist`；它们不进入 App Root Provider，不读取 SecureStore，不复用用户 Query Cache，也不调用用户 API。`/legal/*` 与 `/support` 免登录、关闭 JavaScript 后仍可读。App 通过唯一的 `/public-page` 原生路由和 `react-native-webview` 在应用内承载当前环境的同源页面；路由只接收稳定页面键，容器仅允许 `/legal/*`、`/support/*`，禁用 JavaScript、DOM Storage、第三方 Cookie、文件访问、混合内容、多窗口和任意原生桥接，不传递 Access／Refresh Token。加载、HTTP 错误、离线、重试和原生返回均由容器处理；外部供应商 HTTPS 链接才交给系统浏览器。正式 URL 继续独立公开供应用商店和未安装 App 的用户访问，不能用 App 私有深链冒充商店 URL。
...
当前决策范围只允许已落地的公开静态 H5 和上述无登录态、无桥接的只读原生容器，不包含登录态功能型 H5。移动端已按 `ADR-028` 集成 `expo-updates`，但本地无凭证时默认关闭；正式 APK 构建要求显式注入 Expo Project ID、验签证书，并通过生产 H5 门禁，任一条件缺失都直接失败。新增功能仍先按《H5 与公开页面边界》记录载体决策：只有读为主、轻交互、在线可用且不依赖原生权限或核心敏感写入的模块进入功能型 H5 候选。

出现真实的登录态功能型 H5 需求时，优先建设隔离的 `apps/public-web`，继续使用 OpenAPI 生成 Client 和 Zod 校验；不得直接扩展当前只读容器。必须嵌入时仍只能复用并重新审计单一容器，通过原生发版交付，以短时单用途交换码建立会话，不在 URL、日志或页面脚本暴露长期 Token；新增每项桥接都要单独完成权限、隐私和商店审核。
...
| 页面表单与交互 | React `useState`／Reducer | Task 编辑、对话内确认字段、面板开合 |
...
- 通用 Assistant 路由使用正式 Thread／Message／Proposal Query；页面传入的一次性首轮上下文不得写入长期全局状态。中央 Capture 与 Assistant 图片输入都复用正式 Capture 流程；会话 Provider 只保存恢复所需 ID 与最小展示摘要，不能复制媒体、Question 正文或 Candidate。
...
这是当前目录快照。`notifications` 已建立产品内通知列表与已读状态模块，当前只把确定性 Event Reminder／Task Due 持久化为通知快照；设备 Push、Daily Brief、Project Risk 和 Weekly Review 的通知生成仍未接入。`exports` 等目标模块尚未建立；出现明确业务边界后再拆分，不能先用文档中的目录名假定代码已经存在。
...
Notifications 已拥有产品内持久化列表、来源删除态和首次已读时间；它在列表查询时复用现有 Reminder 确定性计算补齐已到点的 Event／Task 通知。Scheduler、Push、Exports 与自然语言 Search Answer 模块当前不存在；Retention 已拥有账号删除受理、无身份状态镜像和 River 清理 Worker。不能据此假定设备通知投递或导出已经落地。新增模块时必须同时补目录、数据所有权、契约、迁移、测试和本节。
...
Assistant 历史上下文须同时包含正文与服务端处理结果。消息 `completed` 表示已接收，不代表对应 Turn 成功，更不代表业务保存。Context Builder 在当前用户的短 RLS 事务内，针对最近消息窗口一次批量读取对应 Turn 与 Action Proposal 状态，再以系统回执随对应历史用户消息传给编排引擎；不增加模型调用。缺失回执显式标记未知，执行历史不替代对象当前状态，也不能替代独立 Capture 的确认回执。
...
内置账本的收支语义由 Go Trackers Domain 在 Record 创建与更新时统一校验：接受明确的中文历史表达并保存为 `income`／`expense`，金额必须为正。Capture 使用版本化 Prompt 保留实付金额与交易时间；不增加网络字段或数据库列，沿用 RecordValue 与 timestamp 契约。移动端只兼容展示已有中文收支值，不承担新写入的权威转换。
...
新 revision 只复制用户最初提交的 Capture Parts；历史回答依据 Question revision 按时间正序重建为独立来源 Part。Provider Request 将问题、回答和回答来源 ID 成对传递，不把回答扁平化成新的事项，也不因轮数增加而改变顺序。追问轮数不设置固定上限；是否继续提问只取决于当前是否仍缺少形成可确认候选所必需的信息，已经明确回答过的内容不得重复追问。
...
原生客户端把 Access／Refresh Token 和到期时间保存在系统 `SecureStore` 中，冷启动先恢复会话，并在 Access Token 到期前一分钟或受保护请求返回 401 时执行去重的静默 Refresh。网络断开、超时、429 或 5xx 等暂时故障向当前请求返回可恢复错误，但保留设备会话；只有服务端明确返回 `UNAUTHENTICATED`／`REFRESH_TOKEN_INVALID`、账号状态失效或用户主动退出才清理安全存储与用户 Query Cache。回到前台时采用相同策略续期，不得因一次网络失败跳回登录页。
...
important_date_handled_at  仅一次性重要日可写；null 表示未处理
```
...
- `important_date_handled_at` 只能用于 `event_kind=important_date AND recurrence=none`；过期不会自动写入该字段。更新 `start_date`、切换为 `yearly` 或改成普通 Event 时自动清空。
...
- Refresh 的 SECURITY DEFINER 函数在非超级用户属主下先按高熵凭证哈希确定所属用户，再设置局部 RLS 上下文完成锁定、重验及轮换，返回前恢复调用者身份；不向函数属主授予全表 UPDATE／INSERT 绕过策略。CI 必须使用与部署一致的无 BYPASSRLS 迁移角色。
...
`POST /v1/mood-journal/polish` 是独立的心情日记同步候选接口：只接受当前 `blocks_v1` 草稿，并同时检查全局 AI 建议、默认关闭的 `mood_journal_ai_enabled` 单独同意和默认关闭的部署级 `STEWARD_AI_APPROVED_SENSITIVE_CONTENT` Provider 批准门禁。客户端不发送标题、心情、精力、其他日记或上下文；模型必须返回 `mood-journal-polish-result.v1`。Go 在返回前重验原块 ID 覆盖与顺序、服务端生成新增块 ID、禁止删除或重排原块，并确定性比较数字／日期 Token 与链接集合，最后重新执行 `NoteContentBlocksV1` Domain 校验。结果只回填本地草稿并带 `ai_action_id`；创建或更新日记时回传该 ID，服务端校验用户归属、成功状态和 `model_policy=mood_journal_polish` 后才追加 `provenance_refs`。
...
当前没有列表总览专用接口、最近 Capture 列表、自然语言搜索答案、Tracker Schema 建议、情绪追问／回望、Daily Brief、Project Summary 或 Data Summary API。Task 拆分通过 Assistant 的 `task_split` Proposal 批量创建子任务并建立 `related_to` 关系，确认页允许用户删选和修改标题；Task 排期继续复用带目标版本的 `task_update` Proposal，在确认后由同一个 Object Command 写入计划时间。二者都不新增旁路写接口，也不得在用户确认前修改正式数据。其余目标能力实现前必须先补 AI Schema、OpenAPI、确认路径、Eval 和失败降级；不得按本章旧路径手写调用。
...
周历时间轴以响应的用户时区把绝对时刻格式化到分钟，并基于 Event／Task 原始起止时间生成可丢弃的视觉片段。至少 24 小时且跨当地日期的显式定时范围进入独立跨天横条，短跨午夜继续切列；小屏重叠簇聚合、最小时高、全天溢出、08:00 初始定位和时段跳转均只是客户端布局状态，不成为网络字段或本地权威事实。当天详情从响应中原实体去重补充可见续日阅读入口，不改变服务端收录和排序规则；Assistant 继续使用原 `calendar.read` 与待确认 Proposal，无新增 AI Schema、Command 或 API。日程详情隐藏 IANA 时区属性行不代表删除语义字段，读取、编辑、提醒和跨时区计算继续使用 Event `timezone`。
...
| Consents | 心情日记 AI 单独开关已进入 `/me/ai-settings`；其他媒体或敏感正文同意未进入 Public OpenAPI | 单独同意必须形成正式契约，不能只存客户端开关；心情正文还需通过部署级 Provider 敏感数据批准门禁 |
| Reminders／Notifications | 产品内闭环 | `GET /v1/reminders/pending` 提供待提醒读模型；`GET /v1/notifications` 持久化并分页读取已触发的 Event／Task 通知，`POST /v1/notifications/{notification_id}/read` 记录首次已读。设备 Token、Push、Daily Brief、Project Risk、Weekly Review 通知生成和通知设置尚未接入 |
...
sequenceDiagram
    ...
    M->>M: 直接打开 AIConversationSheet 并保存 active Capture 引用
...
App 在 `AIConversationSheet` 内按退避轮询 Operation 与 Capture，关闭浮层后停止高频轮询，但保留当前 App 会话的 active Capture ID，用户再次打开常驻管家入口时继续读取服务端状态。回答 `/v1/capture-questions/{question_id}/answer` 会推进 revision 并再次返回 Operation，客户端在同一对话流替换 active operation 并清空旧 revision 的本地编辑快照。当前没有独立 Part 重试／忽略、服务端最近 Capture 列表或设备推送提示；River 重试依赖 revision 与幂等键避免迟到结果覆盖新结果。正式内容 Query 不订阅 Capture 状态，候选在用户确认前不能出现在主业务页面。
...
sequenceDiagram
    ...
    M->>M: 对话内追加保存结果，显示全局紧凑 3 秒 UndoSnackbar
```
...
确认 UI 从独立路由迁入 `AssistantCaptureFlow` 只改变移动端载体，不改变 `CaptureConfirmationService`、revision 锁、幂等、依赖闭包、事务或 Provenance。OpenAPI、生成 Client、Go Handler 与数据库结构均无需变化。
...
移动端 `UndoSnackbar` 位于全局层并显示 3 秒，按内容收缩且限制最大宽度 420dp；文字可随动态字号换行，撤销按钮保持至少 44×44dp，位置避开安全区和底部一级导航。服务端 `undo_expires_at` 仍保留 10 秒有效期，Activity 版本冲突语义不因客户端展示时间收紧而变化。
...
以下是架构设计过程中使用的决策编号登记。仓库当前由 ADR-028、ADR-029 形成独立文件；ADR-001～027 的规则仍以内嵌在本文的对应章节为准，不能把表中编号引用成不存在的文件。后续只有需要独立记录背景、备选方案、后果和替换条件时才提取 ADR，并同步本表与 `docs/README.md`，不为补齐目录批量创建空文档。
...
| ADR-019 | 官方 Provider SDK 与中立 OrchestrationEngine；初始自研编排已由 ADR-029 取代 |
...
| ADR-025 | 评估 Eino OrchestrationEngine，Checkpoint 非权威；可选实现结论已由 ADR-029 更新 |
...
| ADR-029 | Eino v0.9.19 是唯一编排实现且只承接单 Turn 工具循环，业务状态、确认、审计与行为契约仍由项目拥有 |
...
- [ ] 全局 `AIAssistantFab + AIConversationSheet` 常驻；角标聚合 Open Question、pending Proposal 与会话内 active Capture，中央 Capture 按钮无 AI 角标；Capture 处理与确认在同一对话浮层展示，问题正文和 Candidate 不复制到全局 Store。
...
- [ ] `/legal/*` 与 `/support` 免登录、无占位、关闭 JavaScript 后正文可读；App 内唯一受控容器仅允许同源固定路径，禁用脚本、Token 注入和原生桥接，并覆盖加载、离线、错误、重试与返回。
...
### Assistant 信任边界补充（2026-09-07）

Eino 适配器在调用能力 Handler 前按登记的 JSON Schema 执行运行时校验。每轮独立保存已成功执行的只读工具证据，Proposal 返回值不能给自己提供可信来源。Assistant 收尾事务先写 Tool Call 审计，再以 Turn.user_message_id 与成功只读审计重建建议证据。

确认事务使用保存点区分领域回滚与建议终态提交；更新来源的网络契约包含 assistant_proposal。00054／00055 向前迁移修复已落库的缺失 action，00055 覆盖非超级用户表拥有者执行 FORCE RLS 表回填的情况。

## Assistant 权威交互状态（2026-09-07）

迁移 00056 为 assistant_messages 增加 interaction JSONB，记录结果、澄清及经过校验的选择；它不属于可丢弃的 provider_state。OpenAPI AssistantMessage.interaction 与 CreateTurnRequest 的 clarification_message_id／choice_id 为可选扩展，旧客户端继续使用文字输入。内部对象引用留在服务器，客户端只得到选择 ID 与标签。

CreateTurn 与结果提交按 Thread 行锁串行化，防止旧回复在新输入之后提交。取消和修订复用 action_proposals 的行锁、状态与版本；旧提议终态和新建议与回复一起提交。服务端查询依旧受 RLS 隔离。Go / TypeScript 生成代码、网络 Fixture 与数据库回归同批维护。

独立媒体预处理每个 Capture 最多并发 2 项，结果按输入索引保存。取消停止派发并等待在途调用退出，单项失败仍阻断结构化解析，落库失败不能被内存成功状态掩盖。

## 后台读模型修复补充（2026-09-08）

迁移 00058 向当前环境后台账号明确授予既有三张脱敏统计表的只读权限；默认权限不作为既有表授权的替代。聚合按报表时区刷新昨天和今天，失败不能报告完整新鲜度。后台会话失效由生成 Client 的统一请求适配器通知 SessionProvider，直接管理写请求与查询使用相同清理路径。详细验收与限制见 [后台运营验收](reviews/后台运营验收-2026-09-08.md)。


### 首页与计划按日事项一致性（2026-09-11）

首页与计划“今天／明天”均使用 Today 正式读模型；`day_offset=0/1` 按账号时区在服务端计算查询日。今天保留逾期任务；明天仅收录该日截止、排期或 focus_date 命中的未完成任务，不结转仅因逾期命中的旧任务。全天、跨天、年度重复日程仍由原 Event 投影处理，已处理的一次性重要日继续退出活动读模型。

`tasks` 与 `events` 保留各自服务端顺序，页面先展示任务再展示日程；合计数量由两组已返回实体相加，列表不做分页截断。旧 `counts.total` 继续表示任务数，保持旧客户端与专注场景兼容。页面每分钟刷新当前日期事实，回到前台和手工刷新也会重查；“明天”不保存设备首次打开时的绝对日期。

本次载体为已有 React Native 核心页面，无新权限、SDK、H5 或数据表；沿用 Capture／Assistant 的任务与日程查询、候选、确认及编辑能力，不新增 AI 写入路径。OpenAPI 增加可选参数并同步生成 Client、Zod、Go Handler 和 SQL；现有无参数请求保持兼容。上线须先发布服务端，再通过正式移动端发布流程更新页面，不将后端上线视为手机端已更新。
