...
日记草稿只为中断恢复服务，不是正式事实。当前移动端使用 `expo-secure-store` 保存按用户与目标日记隔离的小型草稿，正式保存后删除；登出与账号删除仍须按统一清理策略覆盖。若正文规模或跨草稿恢复需求超过 Secure Store 的适用边界，再设计加密的通用本地草稿仓库，不能直接把未加密正文落入 SQLite。应用锁偏好同样按账号写入 `SecureStore`，由根导航之上的生命周期 Provider 监听 `AppState`：登录态恢复且偏好未读取完成时先遮挡，从 `active` 进入 `inactive/background` 时立即挂载不含业务内容的全屏原生 Modal，回到前台后通过 `expo-local-authentication` 调用系统面容、指纹或设备凭据，只有成功才移除遮罩。认证取消、失败、超时和锁定都保持遮挡；Web 不持久化开启状态。生物识别模板始终由操作系统处理，App 只接收成功或失败结果；本地解锁不能替代服务端会话、RLS、API 授权或高风险操作的重新认证。
