...
- 当前只有“今天”连接正式 Today Query。“时光、足迹、心情”只渲染无权限、无数据读取、无接口调用的准备状态；“亲友”允许在 `__DEV__` 构建中渲染明确标记的非权威视觉 Fixture，生产构建仍回到准备状态。该预览不改变“Person 不属于 MVP Object”的领域决策，不定义网络 DTO、不调用 Provider、不持久化状态。正式实现亲友前必须通过产品决策与 ADR 引入 Person／Relation／Interaction／CareReminder 边界，完成 OpenAPI、生成 Client、Go Domain、用户隔离、幂等、来源、删除保留、联系人最小权限和 AI Candidate／Proposal 设计；重要日期继续复用 Event，不复制权威日期状态。
