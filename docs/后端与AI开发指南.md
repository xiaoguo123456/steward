...
- 只提供 Schema 错误摘要，不补充新的业务事实。摘要递归到叶子约束，包含可信 Schema 路径、缺失字段、期望类型或枚举，最多三项；不能只回传 `$ref` 包装层，也不能包含实际值、用户正文或未知属性名。
...
- 运营账本统一使用人民币 CNY，官方人民币报价直接保存；输入总量包含缓存部分，普通输入计费必须减去缓存输入。历史美元列仅作迁移和旧镜像兼容，不能用于新版人民币报表。
