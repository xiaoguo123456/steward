...
1. **重要日首页**：进入页面后直接展示“下一个重要日”和按发生日期排序的后续列表，不显示通用介绍、提醒横幅或默认选中详情。最近日期使用单层浅色摘要，保留名称、日期、是否每年重复和剩余天数；后续项目使用紧凑卡片行，点击后从底部打开详情。
2. **新增重要日**：首屏提供不小于 44 dp 的明确“新增”入口；底部面板先选择生日、纪念日、到期日或其他，再填写名称、选择日期、设置每年重复和提醒。生日、纪念日默认开启每年重复，到期日与其他默认只记录一次，但用户可以主动调整。
