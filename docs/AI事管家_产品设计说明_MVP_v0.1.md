...
- Task、Event、Project、Note 和 Record 的创建都必须经过 Capture 理解与确认；管理页只能提供带上下文的 Capture 快捷入口，不能绕过 AI 判断直接创建某一类型。
...
- 没有日期的明确任务可以作为正式 Task 保存；“未安排”是清单筛选，不表示内容仍待确认。
...
- 导航名称、状态文案和按钮若已经表达含义，不再追加同义副文案；设置列表仅在名称无法消除歧义时显示说明。
...
- Project：从“计划 → 清单详情 → 项目筛选 → 管理项目”进入，是聚合 Task、Event、Note 和 Record 的跨内容归属容器，不与待办、日程内容类型平级，也不在计划首屏常驻。
- 账户与设置：只从“首页”左上角小头像进入，不占底栏；包含 Review、AI、通知、隐私、导出和账号。
...
3. AI 消息下优先给出 2～3 个快捷答案；底部保留文字输入、语音按钮和发送按钮。
4. 关闭面板等同于稍后回复，问题仍跨页面保留；需要图片时进入原 Capture 继续补充。
...
- 通用操作默认使用 `Phosphor React Native` 的统一线性图标；当默认库无法提供语义完整的领域图形时，可以整组采用经过评审、授权清晰的成熟图标库，运动模式使用 Pictogrammers Material Design Icons。
- 所有图标都由 `packages/ui` 的 `Icon` 语义适配层控制名称、可视尺寸、基线、粗细和语义色；同一组领域对象不混用不同字形体系，Feature 不直接导入图标包，也不自行绘制或维护 SVG。
...
- 今日：视觉尺寸 40×40、命中区域 44×44 的绿色圆形头像，配合 28 号页面标题和 13 号日期；头像进入账户与设置，不固定放置搜索或通知图标。
...
2. 工作时间：开始、结束、周末偏好。
3. 默认提醒：Event 提醒时间。
4. 第一次 Capture：默认文字，也允许语音、图片或图片组合。
...
1. 顶部：拖动条、标题“记一件事”和关闭按钮。
2. 图片预览区：仅添加图片后出现，可排序、替换、删除。
3. 微信式输入行从左到右固定为：添加图片 `＋`、文本框或“按住说话”、文字／语音切换、发送按钮。语音模式下右侧切换按钮显示为键盘图标。
4. 点击输入行内的 `＋` 后，再弹出“拍照／从相册选择”操作面板，不在主输入面板同时铺开多个媒体按钮。
5. 隐私与限制提示：仅在已选择图片或录音时显示一句简短提示；完整告知放在首次发送媒体的确认面板。

输入面板不显示欢迎文案、输入规则或“当前模式”文字；发送按钮仅在文字、语音或图片至少存在一项时可用。点击关闭按钮或系统返回时，有内容则让用户选择“保存草稿／放弃输入／继续编辑”，无内容则直接关闭。
...
| 状态 | 页面表现 | 操作 |
|---|---|---|
| 未开始 | 输入行显示“按住说话” | 长按开始录音 |
| 录音中 | 红点、时长、“松开完成” | 松开结束录音 |
| 已结束 | 语音条、时长和“自动转写”状态 | 试听、删除、发送整理 |
| 权限拒绝 | 权限说明 | 去设置、改用文字 |

录音结束后立即启动本地／服务端转写准备，但不自动提交 Capture；用户可以继续添加或调整图片，再点击发送。转写内容在处理或确认阶段可修改。
...
“首页”是用户可见名称，Today 继续作为内部聚合领域名与稳定路由。“今天”只作为日期范围或清单筛选使用，不再作为一级页面名称。当前首页不单独展示 Event、Today Records 或最近更新；Event 进入计划的日历视图，Record 进入“打卡”模块。
...
1. **运动首页**：顶部提供“记录”入口；首屏标题为“开始一项运动”，户外跑步／健走／骑行／力量训练以 2×2 的同层级浅色行动卡展示。四张卡使用深品牌绿的成熟运动图形、相同中性底色和一致文字层级，不设置默认选中态，也不以不同颜色制造伪分类；点击任一卡片直接进入对应运动准备页。首页不展示目标、核心指标、开始按钮或本周频次，只在下方用分隔线数据行展示最近两条运动和“全部记录”入口。
2. **运动准备**：户外模式先用平台原生分段控件选择“自由／距离／时长”，健走额外提供“步数”；目标类型与目标数值分离，数值页使用大号等宽数字、步进按钮、成熟跨平台原生滑杆和常用快捷值，不再用多张单选卡同时混排类型与数值。滑杆保留业务步长，但视觉上使用无密集刻度的细轨道与圆形滑块，实际触控高度不小于 44；当前数值已由大号数字表达，不再重复展示“拖动调整”等说明。默认保持低压力的自由运动，达到数值目标时只提醒、不自动结束；GPS 与路线状态合并为紧凑状态行，语音提醒和屏幕常亮使用平台开关，开始按钮固定在底部。力量训练仍选择训练计划并预览动作。当前原型明确提示不会申请真实权限。
3. **户外运动进行中**：地图约占上半屏，路线、起点、当前位置和里程点清晰；下半屏显示距离、时间和两个模式核心指标。暂停与结束使用相同的圆形外框尺寸和触控面积，暂停用品牌色实心表达主操作，结束用危险色浅底描边表达次级危险操作，不以一大一小制造失衡；返回或结束均先展示结束确认。
4. **力量训练进行中**：显示当前动作、动作提示、组数／次数／重量／状态、完成本组按钮和 45 秒休息计时；暂停后禁止完成本组。完成最后一组后进入总结。
5. **运动总结**：按模式显示路线或动作摘要、核心统计和运动感受选择。“保存运动记录”是明确确认动作；当前只保存本地预览，正式接入后才生成满足 Tracker Schema 的 Record。
6. **历史记录**：展示本月次数、累计时长、户外里程和按模式筛选的记录列表；展开项提供可读摘要，不只依赖图表。

通用状态至少覆盖默认、选中、按压、进行中、暂停、结束确认和保存成功。按钮触控区域不小于 44×44 dp；户外进行中数据优先满足阳光下快速扫读，不用小字号堆叠更多专业指标。
...
1. **本周首页**：顶部为“食谱”和健康档案入口，下方使用“本周／发现”页内切换。健康目标入口采用两层扁平状态行，当前目标使用正文强调字号，每日能量换行作为次级信息，档案操作保持正常操作字号，不压缩为说明文字或再包裹卡片。默认展示今天，七天日期栏允许浏览本周其他日期；“计划摄入”使用单层浅中性底板展示能量、蛋白质和膳食纤维，不使用内部框线。早餐、午餐和晚餐分别使用“餐别标题＋换一道＋菜品行”的扁平区段，组内紧凑、组间留白，首页只保留菜名、耗时、难度和计划热量，推荐理由进入菜谱详情。页尾提供“调整当天”和“查看整周”，不放置占据首屏的大型推荐卡或重复餐食卡片。
2. **整周菜单**：按日期分组展示七天餐食摘要，支持展开某天、调整当天和重新生成整周。AI 新菜单以待确认状态展示差异；“采用本周菜单”是明确保存动作，用户返回前保留原菜单，不以浏览预览替代确认。
3. **菜谱发现**：顶部搜索菜名或食材，分类包含快手菜、时令、减脂、增肌和健康控糖；首个推荐位使用克制的横向食物图片，其余菜谱使用图片主导的双列内容流。卡片只保留标题、耗时、难度和一条推荐理由，不叠加点赞、评论、虚假热度或多层标签。
4. **饮食问卷**：四步渐进收集健康目标、身体情况、饮食限制和烹饪条件；顶部显示进度和可返回入口，每步主操作固定且样式一致。普通步骤不使用“可随时修改”等无关安抚、产品能力声明或免责声明式说明填充页面，只保留会影响当前填写、数据用途和安全后果的必要文案；身体数据可以简明说明用途和修改方式，用户声明已确诊糖尿病或正在用药后，再按场景显示风险提示。过敏原使用高可见的明确选择和复核，不只依赖颜色；跳过问卷后仍可浏览发现页，但个性化菜单明确标记为通用推荐。
5. **菜谱详情**：食物图片后依次展示标题、份量／时间／难度、推荐原因、计划营养估算、食材、过敏提示和步骤预览。底部主操作为“加入菜单”，次操作为“开始烹饪”；收藏使用标准图标按钮，不自绘图形。
6. **烹饪模式**：隐藏全局 AI 入口和无关导航，使用大字号显示当前步骤、所需食材和计时器；上一步、下一步保持固定位置，完成后由用户选择标记做过或记录实际摄入，不自动改变正式数据。
7. **购物确认**：按蔬菜、肉蛋奶、主食调味等分组展示缺少食材，合并重复项并允许勾选“家中已有”。主按钮明确为“确认创建购物清单”，成功后再进入既有购物场景。
...
3. **日期与提醒**：日期使用可翻月的完整日历选择，不用难以输入的日期字符串代替主要交互。全天提醒显示明确的当地触发时刻，当前默认 09:00，并可组合选择提前 7 天、提前 1 天和当天。没有选择提醒时明确显示“不提醒”。
...
2. **购物状态**：页面顶部显示待购买数量、已完成数量和一条细进度条。“从食谱添加”作为轻量文字操作放在同一状态区，进入既有食谱流程，不再使用占据整行的底部主按钮。
3. **品类分组**：待购买商品按服务端返回的蔬菜水果、肉蛋奶、主食调味、饮品冲调和其他分组；每项展示名称、数量／规格、可选备注和食谱来源。来源只帮助用户确认用量，不形成 AI／手动创建的层级分类。
4. **完成收纳**：勾选商品后移入“已买到”区，默认保留可见反馈并允许折叠、展开、编辑或移回待购买。全部买齐时显示紧凑完成状态，右上角新增入口始终保留。
...
1. **行程总览**：按“近期行程／历史行程”分组；近期卡片只展示目的地、名称、日期、天数、出发状态、预订数量和行前清单进度，点击进入详情。历史行程使用紧凑列表，不与近期行程争夺视觉层级。
2. **按天安排**：详情默认展示“安排”，用日期切换和纵向时间线组织交通、住宿、活动与会议。时间、地点和事项标题为首要信息，不在首版增加地图、天气或无数据支撑的智能提示。
3. **预订信息**：集中展示交通和住宿确认状态；票号、订单附件与联系人等敏感或次级信息按需进入详情，不在列表平铺。
4. **行前清单**：展示确定性的完成进度和可勾选 Task，未完成项优先；完成操作只更新对应 Task，不改变 Event 或 Project 状态。
5. **数据关系**：整体行程由 Project 组织，交通／住宿／活动由 Event 表达，准备项由 Task 表达，票据和补充资料由 Note、附件及 Relation 表达。AI 导入截图或文字后只能生成待确认候选，用户确认后才写入正式对象。
6. **延后能力**：地图路线、订单邮件自动导入、多人协作和离线资料包后续单独设计；首版不使用不可点击入口或说明文案占位。
...
“今天要做”的未完成数量紧跟在标题后，以品牌浅色底和高对比文字显示为“N 项”；不得作为远离标题的低对比辅助文字放在区域右侧。
...
- “待安排”使用单独的高优先级行，显示尚未安排日期的 Task 数量和简短说明；点击后进入完整待安排列表。
- TaskList 统一放在“清单”标题下成组显示，只展示名称、未完成数量和进入箭头；不按 AI／用户创建来源拆分“智能清单／我的清单”，标签不在计划首屏形成独立区域。
- 点击时间入口或 TaskList 后进入对应紧凑任务列表；状态、关键词、标签和 Project 筛选在详情层渐进提供，不占据计划首屏。
- Project 是待办、日程、重要日期、目标节点和笔记可共享的归属容器，不与内容类型平级，也不在计划首屏常驻；从具体清单或时间列表的 Project 筛选进入项目选择和管理。
- TaskList 的创建、重命名、排序与归档进入管理面板；工作、生活等示例名称只属于 Mock，不作为固定系统分类写入正式规则。
- 页面只读取已确认的正式日期事实，并以 TaskList、Project 作为组织与筛选信息；Capture 处理状态不进入本页。
- 日历是计划的独立二级页面，默认显示含今天的 7 天周视图；周期翻页独占第一行并保持标题真正居中，“今天”和“周／月”位于第二行。星期标题、日期格与日期数字圆心必须复用同一个等宽七列基准，禁止分别使用不同内边距或最小列宽。单一安排使用清晰的短横条，多类型安排使用 4～5 px 实色点，不能使用难以辨认的 3 px 弱提示。周／月切换提供可见的“展开月历／收起月历”按钮和方向箭头，命中区域不小于 44 dp，不得把滑动手势作为唯一入口；展开时显示完整 6 周月格，收起时回到选中日期所在周，且始终保留选中日期。选择日期后，下方按“全天与重要日期／时间安排”分区展示当日内容。
- 生日和纪念日使用暖红色重要日期图标；Project 目标日期和里程碑使用琥珀色旗标；普通日程使用紫灰日历图标；待办使用品牌绿勾选图标。每种类型同时显示文字，不能只依赖颜色。
...
“未安排”表示任务已经确认，但没有截止、计划或关注日期。它是筛选条件，不使用警告样式，也不要求用户先安排日期才能保留任务。

列表排序优先展示未完成任务，再按计划／截止时间、优先级和创建时间排列。来源、AI 解释和次要操作进入详情，不挤占首屏。

## 10.3 TaskList 与空状态

- TaskList 支持新建、重命名、排序、归档和删除；删除非空清单先选择迁移目标。
- 新建 Task 默认进入默认清单；从某个 TaskList 新建时预选当前清单。
- 空 TaskList 文案为“这里还没有任务”，主操作为“新建任务”。
...
更多菜单包含：复制为新日程、删除。删除进入最近删除并取消未发送提醒。Event 不设置完成状态；已经结束只影响 Today 分组，不改变实体状态。
...
“转换为 Task”打开候选确认页：Note 标题映射为 Task 标题、正文映射为描述；用户选择是否保留原 Note。默认保留并建立 `related_to`。
...
- 从“计划 → 清单详情 → 项目筛选 → 管理项目”进入。搜索位于项目选择器，管理页不提供手工新建 Project。
- 项目选择器和管理页都可以提供“告诉 AI 一个新目标”；它打开统一 Capture，不预设最终类型。AI 只有在目标包含多个步骤、需要持续推进时才生成 Project Candidate，单步事项仍生成 Task Candidate。
...
- 常用项目在项目选择器中优先显示；项目行的更多操作集中提供打开、设为常用和归档，已归档项目进入独立入口。
...
更多菜单必须覆盖完整状态机：Active 可暂停、完成或归档；Paused 可恢复、完成或归档；Completed 可重新打开或归档；Archived 只允许恢复到归档前状态。标记完成时先显示未完成 Task 数，不自动完成这些 Task。
...
- 默认首屏先显示“今日待打卡”，只收录当日到期且尚未完成的 Record；完成后该项即时退出这个分区。其后显示“我的打卡”，活跃 Tracker 按最近记录时间排序。
- 睡眠、饮水、阅读、体重等都使用 Tracker／Record 表达，不新增独立 Habit 实体；运动和番茄钟完成后可以写入对应 Tracker，并进入趋势统计。
- 每项显示名称、最近值、单位、更新时间与紧凑趋势；待打卡项的“打卡”按钮直接打开该 Tracker Schema 对应的录入面板，不先进入无关的中间页。
- “管理打卡”底部面板集中处理 Tracker 排序、归档、恢复和创建；已归档 Tracker 只在此处渐进展开，不占用首页分类位置。
- 打卡保存后即时更新最近值和完成状态；记账等非定期 Record 不强制进入“今日待打卡”。所有正式写入仍需通过 Schema 校验并保留来源。
...
Schema 编辑列表包含字段名、类型、单位、是否必填和删除。拖动调整字段顺序。
...
少于 2 条记录时隐藏趋势结论，并说明“至少需要 2 条记录”。图表必须同时提供列表或可读数据摘要，不能只依赖视觉曲线。
...
1. **AI 周总结**：先用一段短文概括本周最有价值的变化，明确由任务、专注与打卡数据整理；不生成欢迎语、鼓励语或没有依据的情绪判断。
2. **数据概览**：使用单层紧凑信息条展示固定口径的核心数据，不使用大号英雄数字或多张指标卡片。指标由确定性代码计算，AI 不重新计算。
...
页面状态包括：生成中使用内容骨架；数据不足时保留确定性概览并省略无依据观察；AI 失败时仍展示确定性概览和重试入口；数据变化后显示“基础数据已变化”，提供“重新生成”并保留旧快照；历史周可以继续查看当时的报告内容。
...
用户只从“首页”左上角头像打开账户与设置页，不占用底部导航。页面分组：

- 本周 Review：使用白底摘要行展示周期、一个关键数字和一个观察；整行可点击，右侧只显示“查看”，不使用大面积品牌色卡片或文字箭头组合。
- 使用习惯。
- AI 设置和长期偏好。
- 通知设置。
- 数据与隐私。
...
设置分组默认只显示图标和名称。能由名称直接理解的入口不添加解释性副标题，降低扫描成本。
...
- 按删除日期倒序显示 Object、Tracker 和 Project，展示永久删除剩余天数。
- 单项支持恢复和永久删除。
- 恢复存在 Project、Tracker 或 Relation 依赖冲突时，先显示影响并要求选择新的归属。
- 永久删除使用二次确认，不提供撤销。
...
- 结构化数据：Object、Relation、Tracker、Record、偏好和 Activity Log，默认选中。
- 原始图片，默认不选中。
- 原始录音，默认不选中。
- 转写和 OCR，默认选中。

提交后进入导出详情，状态为：等待中、生成中、可下载、失败、已过期。
...
| SET-00 | `/settings` | 账户与设置 |
| SET-01 | `/settings/preferences` | 使用习惯 |
| SET-02 | `/settings/ai` | AI 设置 |
| SET-03 | `/settings/notifications` | 通知设置 |
| SET-04 | `/settings/privacy` | 隐私与数据 |
| SET-05 | `/settings/deleted` | 最近删除 |
| SET-06 | `/settings/export` | 数据导出 |
| SET-07 | `/settings/captures` | 原始 Capture 列表 |
| SET-08 | `/settings/captures/:id` | 原始 Capture 详情 |
| SET-09 | `/settings/account/delete` | 删除账号 |
...
- [ ] 一级导航固定为“首页、计划、笔记、打卡”；只有首页左上角小头像进入账户与设置，Project 从计划的项目范围入口进入。
- [ ] 每页只有一个视觉主操作。
- [ ] 列表条目默认只有一个显式下一步；次要操作和说明通过渐进展开提供。
- [ ] 账户与设置页的周报使用轻量摘要入口，不使用整块高饱和色卡或重复的“完整回顾＋箭头”提示。
- [ ] 所有页面都有加载、空、失败和无权限状态。
...
- [ ] 计划首屏显示“今天／明天／已完成”、独立“待安排”和统一 TaskList，不出现“智能清单／我的清单”来源分组；“未安排”是筛选，不是状态。
- [ ] 日历周／月视图可到达，生日、纪念日、Task、Event 和 Project 目标日期均有明确类型且按日期正确收录。
...
- [ ] 通知权限被拒绝后，产品内提醒仍可查看。
- [ ] 单媒体删除、完整 Capture 删除、数据导出和账号删除均有进度、失败与不可恢复说明。
