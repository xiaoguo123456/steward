...
2. **运动准备**：户外模式先用平台原生分段控件选择“自由／距离／时长”，健走额外提供“步数”；目标类型与目标数值分离，数值页使用大号等宽数字、步进按钮、平台原生滑杆和常用快捷值，不再用多张单选卡同时混排类型与数值。默认保持低压力的自由运动，达到数值目标时只提醒、不自动结束；GPS 与路线状态合并为紧凑状态行，语音提醒和屏幕常亮使用平台开关，开始按钮固定在底部。力量训练仍选择训练计划并预览动作。当前原型明确提示不会申请真实权限。
...
1. **本周首页**：顶部为“食谱”和健康档案入口，下方使用“本周／发现”页内切换。默认展示今天，七天日期栏允许浏览本周其他日期；“计划摄入”使用单层浅中性底板展示能量、蛋白质和膳食纤维，不使用内部框线。早餐、午餐和晚餐分别使用“餐别标题＋换一道＋菜品行”的扁平区段，组内紧凑、组间留白，首页只保留菜名、耗时、难度和计划热量，推荐理由进入菜谱详情。页尾提供“调整当天”和“查看整周”，不放置占据首屏的大型推荐卡或重复餐食卡片。
