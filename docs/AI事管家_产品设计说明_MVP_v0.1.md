...
2. **运动准备**：户外模式选择不限目标、距离、时间或步数目标，并显示模拟定位、路线记录、里程播报和屏幕常亮状态；力量训练选择训练计划并预览动作。当前原型明确提示不会申请真实权限。
3. **户外运动进行中**：地图约占上半屏，路线、起点、当前位置和里程点清晰；下半屏显示距离、时间和两个模式核心指标。暂停为主操作，结束为次级危险操作；返回或结束均先展示结束确认。
