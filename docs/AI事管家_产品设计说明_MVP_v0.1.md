...
- 使用 `Phosphor React Native` 的统一线性图标，由 `packages/ui` 的 `Icon` 适配层控制名称、大小、粗细和语义色；Feature 不直接导入图标包。
...
1. **运动首页**：顶部提供“记录”入口；首屏显示“今天想怎么动”、户外跑步／健走／骑行／力量训练 2×2 模式选择、所选模式的单一开始按钮、本周 2／3 次习惯进度和最近一条运动。模式卡片只承载选择，不嵌套第二层操作。
