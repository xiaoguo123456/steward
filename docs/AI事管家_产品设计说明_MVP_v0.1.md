...
“首页”是用户可见名称，Today 继续作为内部聚合领域名与稳定路由。“今天”只作为日期范围或清单筛选使用，不再作为一级页面名称。当前首页不单独展示 Event、Today Records 或最近更新；Event 进入计划的日历视图，Record 进入数据模块。

## 9.2 预留快捷入口

- 固定为每行 4 个、共 2 行的金刚位结构，使用图标和短标签。
- 8 个金刚位整体放在单层浅色功能区中，通过背景、内边距和区块间距与 Today Tasks 隔离；不使用投影、描边或嵌套卡片增加装饰层级。
- 功能未确定前显示“入口一”至“入口八”的占位内容，保持不可点击，不能伪装成已经可用的入口。
- 后续确定功能时复用当前位置与触控规格，不改变首页主体分区顺序。
...
- 顶部：标题“计划”、当前内容摘要和“筛选”操作。
- 主视图切换只包含“列表、日历”，使用紧凑文字 Tab 和 2px 选中线；按钮保留 44 高命中区，但不使用占满整行的大面积分段背景。日历不再放在右上角，也不以弹窗出现。
- Project 是待办、日程、重要日期、目标节点和笔记可共享的归属容器，不与内容类型平级。计划页在主切换下方显示横向常用项目快捷栏；“更多”打开带搜索、最近使用和无项目视图的范围选择器，再从选择器右上角进入项目管理。
- 列表把已确认 Task、Event、`important_date` 和 Project `target_date` 放在同一连续时间流中，通过类型图标与辅助文字区分；默认按“今天／接下来／未安排”组织。内容类型、Task 状态和所属 TaskList 进入筛选面板，不再常驻第二排筛选。
- TaskList 的创建、重命名、排序与归档进入管理面板；工作、生活等名称不作为固定系统分类写死在首屏。
- 搜索入口与筛选共用同一上下文面板，输入关键词后与状态、TaskList、Project 条件共同生效；清除筛选同时清除关键词。
- 页面只读取已确认的正式日期事实，并以 TaskList、Project 作为组织与筛选信息；Capture 处理状态不进入本页。
- 日历默认显示含今天的 7 天周视图；周期翻页独占第一行并保持标题真正居中，“今天”和“周／月”位于第二行。星期标题、日期格与日期数字圆心必须复用同一个等宽七列基准，禁止分别使用不同内边距或最小列宽。单一安排使用清晰的短横条，多类型安排使用 4～5 px 实色点，不能使用难以辨认的 3 px 弱提示。周／月切换提供可见的“展开月历／收起月历”按钮和方向箭头，命中区域不小于 44 dp，不得把滑动手势作为唯一入口；展开时显示完整 6 周月格，收起时回到选中日期所在周，且始终保留选中日期。选择日期后，下方按“全天与重要日期／时间安排”分区展示当日内容。
...
- 从“计划 → 项目快捷栏 → 更多 → 管理项目”进入。搜索位于项目选择器，管理页不提供手工新建 Project。
...
- 常用项目可以固定到计划页快捷栏；项目行的更多操作集中提供打开、设为常用和归档，已归档项目进入独立入口。
...
- 顶部只显示标题、完成摘要和“管理”，不设置“全部／今日待记录／已归档”常驻 Tab；新建 Tracker 仍统一从 `＋` 描述需求或在管理页进入创建流程。
- 默认首屏先显示“今日待记录”，只收录当日到期且尚未完成的 Record；完成后该项即时退出这个分区。其后显示“我的记录”，活跃 Tracker 按最近记录时间排序。
- 每项显示名称、最近值、单位、更新时间与紧凑趋势；待记录项的“记录”按钮直接打开该 Tracker Schema 对应的录入面板，不先进入无关的中间页。
- “管理记录”底部面板集中处理 Tracker 排序、归档、恢复和创建；已归档 Tracker 只在此处渐进展开，不占用首页分类位置。
- 记录保存后即时更新最近值和完成状态；所有正式写入仍需通过 Schema 校验并保留来源。
...
- [ ] 一级导航固定为“首页、计划、笔记、数据”；只有首页左上角小头像进入账户与设置，Project 从计划的项目范围入口进入。
- [ ] 每页只有一个视觉主操作。
...
- [ ] 首页首屏显示 4×2 预留快捷入口、最多 4 项 Today Tasks 和今日提醒；任务可在当前页展开与收起。
- [ ] 计划用“列表／日历”切换同一批已确认事实；“未安排”是筛选，不是状态。
- [ ] 日历周／月视图可到达，生日、纪念日、Task、Event 和 Project 目标日期均有明确类型且按日期正确收录。
- [ ] 笔记与数据均有独立一级页面，且只展示已确认内容。
- [ ] Project 风险显示具体原因。
- [ ] 图表同时有文本或列表替代。
- [ ] 自然语言搜索回答包含可点击来源。
