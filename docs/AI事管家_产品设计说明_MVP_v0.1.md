...
| 导航 | 核心任务 | 默认内容 |
|---|---|---|
| 首页 | 决定现在和今天做什么 | 当天 Event、Task、Brief、待记录项 |
...
首屏优先让用户看到下一项 Event 和最高优先级 Task，不能被大面积欢迎文案挤出屏幕。

## 9.2 Event 区域

- 全天 Event 使用紧凑横条，显示日期范围和 Project。
- Now / Next 使用时间轴样式，当前 Event 标记“进行中”，无结束时间时标记“已开始”。
- 接下来 Event 显示距离开始时间、开始时间和地点；点击整行进入 Event 详情，不额外放置“准备事项”按钮。
- 已结束 Event 收入折叠分组。
...
- 只展示今天尚未记录的活跃 Tracker 和已记录状态。
- 点击未记录项打开快速录入底部面板。
- 多字段 Tracker 使用完整录入页，避免在窄面板中横向滚动。
...
- 日历默认显示含今天的 7 天周视图；周期翻页独占第一行并保持标题真正居中，“今天”和“周／月”位于第二行。星期标题、日期格与日期数字圆心必须复用同一个等宽七列基准，禁止分别使用不同内边距或最小列宽。单一安排使用清晰的短横条，多类型安排使用 4～5 px 实色点，不能使用难以辨认的 3 px 弱提示。切换月视图时展开完整 6 周月格且不改变选中日期；选择日期后，下方按“全天与重要日期／时间安排”分区展示当日内容。
...
用户只从“今日”左上角头像打开账户与设置页，不占用底部导航。页面分组：
...
- [ ] Today 首屏能看到下一 Event 和最高优先 Task。
