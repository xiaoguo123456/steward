...
AI 主动向用户提问时，采用“有事才出现”的独立悬浮球：52×52 的绿色渐变圆形按钮，使用白色星芒 AI 图标、柔和绿色投影和右上角红色数量角标，与中央纯色 `＋` 明确区分。角标只显示尚未处理的待答、处理完成或待确认总数，超过 9 显示 `9+`；一级页入口固定在底栏右上方。无待答、处理或待确认事项时整个入口消失。中央 `＋` 只负责新增，不显示红点、数字或 AI 状态。悬浮球不使用宠物形象、持续呼吸动画或自动展开气泡。
...
- AI 元素只在需要解释来源、待答问题或建议时出现；仅全局 AI 悬浮球允许使用受控的绿色渐变与柔和光晕，其余 AI 标签和面板保持纯色、轻量。
...
| `ai_start` | `#47D68A` | AI 悬浮球渐变起点 |
| `ai_end` | `#10B981` | AI 悬浮球渐变终点 |
...
- 顶部只显示标题、内容摘要和“筛选”，不设置常驻分类 Tab；新增统一从底栏中央 `＋` 进入 Capture。
- 默认视图固定为“置顶”和“最近更新”两个连续分区。置顶 Note 在前，其余按最近更新时间倒序，避免用户先判断分类才能浏览。
- 标签与 Project 属于低频缩小范围操作，进入“筛选笔记”底部面板；每次各选一个条件。应用后只在标题下方显示可单独移除的条件标签，不保留第二排导航。
- 默认列表项显示标题、一行正文摘要、更新时间、标签以及图片或附件标记。点击进入 Note 详情；更多菜单提供置顶或取消置顶、调整标签和删除，Project 移动与转换为 Task 放在详情内。
- 页面只展示已确认 Note。Capture 原始输入、转写或 OCR 在确认保存前不作为 Note 出现。
- 无 Note 时显示“还没有笔记”，主操作为“记一件事”；筛选无结果时显示“没有符合条件的笔记”，主操作为“清除筛选”。
...
每条观察和建议有来源。数据变化后显示“基础数据已变化”，提供“重新生成”。旧快照仍可查看。
