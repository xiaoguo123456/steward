...
AI 主动向用户提问时，采用“有事才出现”的独立悬浮入口：使用约 56×56 的暖色叠页管家形象，由杏橙前页、日光黄后页和一对清晰的大眼睛组成，以错位纸页表达“接收并整理事务”。形象保持透明背景，不增加绿色圆底、星芒、对勾、嘴或四肢；右上角使用红色数量角标，与中央纯色 `＋` 明确区分。角标只显示尚未处理的待答、处理完成或待确认总数，超过 9 显示 `9+`；一级页入口固定在底栏右上方。无待答、处理或待确认事项时整个入口消失。中央 `＋` 只负责新增，不显示红点、数字或 AI 状态。形象不使用宠物设定、持续呼吸动画或自动展开气泡。
