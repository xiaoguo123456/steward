...
1. **本周首页**：顶部为“食谱”和健康档案入口，下方使用“本周／发现”页内切换。默认展示今天，七天日期栏允许浏览本周其他日期；紧凑的“计划摄入”区域展示能量、蛋白质和膳食纤维，早餐／午餐／晚餐使用带真实图片的横向内容行。每餐可以“换一道”，页尾提供“调整当天”和“查看整周”，不放置占据首屏的大型推荐卡。
...
4. **饮食问卷**：四步渐进收集健康目标、身体情况、饮食限制和烹饪条件；顶部显示进度和可返回入口，每步主操作固定且样式一致。过敏原使用高可见的明确选择和复核，不只依赖颜色；跳过问卷后仍可浏览发现页，但个性化菜单明确标记为通用推荐。
