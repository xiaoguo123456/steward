...
选菜在服务端（`modules/recipes/suggest.go`），确定性代码，不调用模型。
`GET /v1/meal-plans/suggestion?seed=N` 返回**预览**，不落库；
用户点「采用本周菜单」走 `confirmMealPlan` 才写入。

三类规则，优先级从高到低：

1. **过敏原与忌口**。在 SQL 里过滤（`ListRecipeCandidates`），任何情况下都不放宽。
   可选菜再少也宁可留空——为了排满一周而放宽过敏原，是这个功能唯一能伤到人的方式。
   最长烹饪时间是偏好，候选不足七道时会放宽并在 notes 里说明；过敏原没有对应分支，也不要加。
2. **一周不重样**，跨餐次也算。候选不够才允许重复，且必须出现在 notes 里。
3. **目标匹配**。按营养在候选池内的**排名**打分，不用绝对阈值：
   导入内容的营养是按食材表估算的，绝对值不可靠，相对高低大体成立。

两个容易写错的地方：

- **打分不能是单调的「越低越好」**。减脂那条最早写成「热量排名越低分越高」，
  实测选出的菜平均每餐 114 kcal、一天三百出头——照着吃不了。
  改成向一个目标分位靠拢（`nearTarget`），真正拉开差距的是蛋白密度。
- **并列的值必须给同一个排名**。`percentiles` 最初直接用下标当百分位，
  值相同的菜按 ID 分先后。而 `duration_minutes` 是整数、几千道菜都正好 20 分钟，
  「快」这一项就变成了按 ID 抽签。

时段按早→午→晚处理，因为早餐候选最少（约 940 道，午晚各 2600），
**最受限的先挑**；反过来午晚餐会把早餐仅有的那些菜先占走。

客户端只做单格「换一道」（在已取回的菜谱里换，要即时反馈）；
整周生成一律走服务端——客户端手里只有前 100 条菜谱，
在那里面轮播既筛不掉过敏原也谈不上按目标选菜。
