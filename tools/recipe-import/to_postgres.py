...
CONTENT_VERSION = "lanfan-2026-08.3"
...
# 来源餐段只作为输入，不能直接等同于平台推荐结论。咖喱类在中国用户的常规
# 早餐语境下明显偏午晚餐；即使来源带了「早餐」，也不进入早餐自动菜单。
# 来源标签仍完整保存在 tags 中，用户也仍可在发现页浏览并手动加入菜单。
BREAKFAST_EXCLUDED_TAGS = {"咖喱"}
...
def build_meal_slots(categories: list[str]) -> list[str]:
    tagset = set(categories)
    slots = {MEAL_TAGS[c] for c in tagset if c in MEAL_TAGS}
    if "breakfast" in slots and tagset & BREAKFAST_EXCLUDED_TAGS:
        slots.discard("breakfast")
