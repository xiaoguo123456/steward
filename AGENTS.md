...
6. 涉及登录注册、隐私权限、第三方 SDK／Provider、账号或数据删除、帮助支持、公开分享、运营活动、订阅付费、功能型 H5、远程更新或应用商店发布时，阅读 `docs/H5与公开页面边界.md`。

不得只根据页面截图、单个接口、旧 PRD 或局部代码猜测完整业务规则。

## H5 与合规提醒门禁

- 命中 `docs/H5与公开页面边界.md` 的开发触发项时，开始编码前必须主动提醒用户：本次需要新增或更新哪些 H5 页面、个人信息／第三方清单、单独同意和应用商店申报。
- 法律文本、帮助中心、公开活动和可分享只读内容优先评估 H5；首页、任务、Capture、AI 确认、媒体／定位、账号安全和隐私控制等核心状态流程默认保持原生。
- 每个新增或重做的用户功能在设计前必须记录载体决策：公开／功能型 H5、React Native，或需要应用商店原生版本。不能仅以“减少发版”为理由选择 H5；一旦依赖原生权限、后台任务、可靠离线、高频核心状态或敏感写操作，默认保持原生。
- H5、Feature Flag 和 OTA 都不能用于绕过应用商店审核。项目已按 `ADR-028` 接入 `expo-updates`，但只有包含验签证书的新原生包可以接收更新；OTA 发布必须通过仓库工作流完成运行时兼容、签名、变更范围、灰度和回滚检查，禁止开发者从本机直接发布生产更新。
- 运营主体、联系方式、法务文字、第三方处理者或保留策略未确认时，只能完成结构和技术方案，不得发布占位协议、虚构联系信息或声称已经合规。
- H5 只是内容载体，不能绕过正式 OpenAPI、生成 Client、用户隔离、幂等、限流和 Domain 状态机。账号删除等写操作仍必须完成跨端契约闭环。
...
- 功能分支合并到 `main` 并完成必要验证后，默认删除对应的本地功能分支，不长期保留已经完成的 `codex/*` 分支。
- 删除前必须确认分支提交已被 `main` 完整包含、关联工作树没有未提交变更；仍被工作树占用时，先将该工作树安全切换到分离状态或其他有效分支。
- 已推送的远端功能分支在确认没有其他协作者依赖、且不属于受保护分支后同步删除。
- 存在未合并提交、未提交变更、未完成任务或明确要求保留时不得删除，也不得使用强制删除掩盖未合并内容。
