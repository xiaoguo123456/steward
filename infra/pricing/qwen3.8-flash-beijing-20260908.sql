...
-- USD 账本按核验日 1 USD = CNY 6.7804 固定折算（非实时外汇结算）。
-- 汇率来源：https://www.safe.gov.cn/AppStructured/hlw/RMBQuery.do?COLLCC=546877760
...
INSERT INTO ai_model_prices
 (id, provider, model, usage_unit, unit_size, unit_price_usd, effective_from)
SELECT 'aip_qwen38_bj_20260908_' || usage_unit, 'openai', 'qwen3.8-flash', usage_unit,
       1000000, round(cny_price / 6.7804, 8), '2026-09-08T00:00:00+08:00'::timestamptz
...
BEGIN
 IF (SELECT count(*) FROM ai_model_prices p JOIN
      ...
      WHERE p.provider='openai' AND p.model='qwen3.8-flash' AND p.usage_unit=expected.unit
       AND p.unit_size=1000000 AND p.unit_price_usd=round(expected.cny_price/6.7804,8)
...
END $$;
COMMIT;
SELECT model, usage_unit, unit_size::text, unit_price_usd::text, effective_from
